...
(function(){
  ...
  const PROFILE_PRESET_KEY = 'a11y-widget-profile-preset:v1';
  const FAVORITES_KEY = 'a11y-widget-favorites:v1';
  ...
  const sideToggleTexts = sideToggleBtn ? {
    left: sideToggleBtn.dataset.textLeft || 'À gauche',
    right: sideToggleBtn.dataset.textRight || 'À droite'
  } : null;
  const sideToggleStatuses = sideToggleBtn ? {
    left: sideToggleBtn.dataset.statusLeft || 'Le panneau est placé à gauche.',
    right: sideToggleBtn.dataset.statusRight || 'Le panneau est placé à droite.'
  } : null;
  const sideToggleText = sideToggleBtn ? sideToggleBtn.querySelector('[data-role="side-toggle-text"]') : null;
  const positionStatus = document.querySelector('[data-role="panel-position-status"]');
  const infoTrigger = document.getElementById('a11y-info-trigger');
  const infoBackdrop = document.getElementById('a11y-info-backdrop');
  ...
  const infoSearchForm = infoDialog ? infoDialog.querySelector('[data-role="info-search-form"]') : null;
  const infoSearchInput = infoDialog ? infoDialog.querySelector('[data-role="info-search-input"]') : null;
  const infoSearchEmpty = infoDialog ? infoDialog.querySelector('[data-role="info-search-empty"]') : null;
  const infoSearchStatus = infoDialog ? infoDialog.querySelector('[data-role="info-search-status"]') : null;
  const infoSearchResultSingular = infoSearchForm ? (infoSearchForm.dataset.resultSingular || '%d rubrique trouvée dans l’aide.') : '%d rubrique trouvée dans l’aide.';
  const infoSearchResultPlural = infoSearchForm ? (infoSearchForm.dataset.resultPlural || '%d rubriques trouvées dans l’aide.') : '%d rubriques trouvées dans l’aide.';
  const infoSearchEmptyText = infoSearchForm ? (infoSearchForm.dataset.emptyText || 'Aucun tutoriel ne correspond à cette recherche.') : 'Aucun tutoriel ne correspond à cette recherche.';
  ...
  const sectionAccordion = document.querySelector('[data-role="section-accordion"]');
  const sectionToggles = sectionAccordion ? Array.from(sectionAccordion.querySelectorAll('[data-role="section-toggle"]')) : [];
  ...
  const activeSummary = document.querySelector('[data-role="active-summary"]');
  const activeSummaryCount = activeSummary ? activeSummary.querySelector('[data-role="active-summary-count"]') : null;
  const activeSummaryLabel = activeSummary ? activeSummary.querySelector('[data-role="active-summary-label"]') : null;
  const activeResetBtn = document.getElementById('a11y-active-reset');
  const activeUndoBtn = document.getElementById('a11y-active-undo');
  const favoritesPanel = document.querySelector('[data-role="favorites-panel"]');
  const favoritesList = favoritesPanel ? favoritesPanel.querySelector('[data-role="favorites-list"]') : null;
  const favoritesEmpty = favoritesPanel ? favoritesPanel.querySelector('[data-role="favorites-empty"]') : null;
  const favoritesCount = favoritesPanel ? favoritesPanel.querySelector('[data-role="favorites-count"]') : null;
  const footerUtilityToggle = document.getElementById('a11y-footer-utility-toggle');
  const footerUtilitySectionId = footerUtilityToggle ? (footerUtilityToggle.dataset.sectionId || '') : '';
  ...
  const shortcutsBackdrop = document.getElementById('a11y-shortcuts-backdrop');
  const tutorialCloseBtn = document.getElementById('a11y-tutorial-close');
  const optionsToggle = document.getElementById('a11y-options-toggle');
  const optionsView = document.getElementById('a11y-options-view');
  const structureToggle = document.getElementById('a11y-structure-toggle');
  const structureView = document.getElementById('a11y-structure-view');
  const structureCloseBtn = document.getElementById('a11y-structure-close');
  ...
  const structureViewContent = structureView ? structureView.querySelector('[data-role="structure-view-content"]') : null;
  const structureViewEmpty = structureView ? structureView.querySelector('[data-role="structure-view-empty"]') : null;
  const infoGlossaryContent = infoDialog ? infoDialog.querySelector('[data-role="info-glossary-content"]') : null;
  const infoGlossaryEmpty = infoDialog ? infoDialog.querySelector('[data-role="info-glossary-empty"]') : null;
  ...
  const tutorialTriggerOpenLabel = tutorialToggle
    ? (tutorialToggle.dataset.openLabel || tutorialToggle.getAttribute('aria-label') || '')
    : '';
  const tutorialTriggerCloseLabel = tutorialToggle
    ? (tutorialToggle.dataset.closeLabel || '')
    : '';
  const isMacOS = typeof navigator !== 'undefined' && /Mac|iPhone|iPad|iPod/.test((navigator.platform || navigator.userAgent || ''));
  let structureViewRendered = false;
  let infoGlossaryRendered = false;
  let shortcutsDialogIsOpen = false;
  let shortcutsDialogHideTimer = null;
  ...
  function normalizeFavoriteKeys(raw){
    ...
    return keys.slice(0, FAVORITES_LIMIT);
  }
  ...
  function persistFavoriteKeys(){
    try { localStorage.setItem(FAVORITES_KEY, JSON.stringify(favoriteKeys)); } catch(err){ /* ignore */ }
  }

  function collectScalarSettings(settings, target){
    if(!settings || typeof settings !== 'object' || !target){ return; }
  ...
  function buildFeatureSearchText(feature, section){
    const parts = [];
    if(section){
      parts.push(section.title, section.slug, section.id);
    }
    if(feature){
      parts.push(feature.label, feature.hint, feature.aria_label, feature.slug, feature.template);
      collectScalarSettings(feature.settings, parts);
    }
    return parts.filter(value => typeof value === 'string' && value.trim()).join(' ');
  }

  function shouldCatalogFeature(feature){
    if(!feature || typeof feature.slug !== 'string' || !feature.slug){ return false; }
    if(typeof feature.label !== 'string' || !feature.label){ return false; }
    return !isPassiveFeature(feature);
  }

  const featureCatalog = new Map();
  function registerFeatureCatalogEntry(feature, section){
    if(!shouldCatalogFeature(feature)){ return; }
    if(featureCatalog.has(feature.slug)){ return; }
    featureCatalog.set(feature.slug, {
      key: feature.slug,
      feature,
      section,
      sectionTitle: section && typeof section.title === 'string' ? section.title : '',
      searchText: buildFeatureSearchText(feature, section),
    });
  }
  ...
  function buildFeatureCatalog(){
    sectionsData.forEach(section => {
      ...
      section.features.forEach(feature => {
        if(!feature){ return; }
        if(Array.isArray(feature.children) && feature.children.length){
          feature.children.forEach(child => registerFeatureCatalogEntry(child, section));
          return;
        }
        registerFeatureCatalogEntry(feature, section);
      });
    });
  }

  buildFeatureCatalog();
  let favoriteKeys = normalizeFavoriteKeys(loadFavoriteKeys().filter(key => featureCatalog.has(key)));
  ...
  function isDedicatedGlossaryFeature(feature){
    return !!(feature && typeof feature.template === 'string' && feature.template === 'dictionary-glossary');
  }

  function isDedicatedStructureFeature(feature){
    return !!(feature && typeof feature.template === 'string' && feature.template === 'page-structure');
  }

  function isPassiveFeature(feature){
    if(!feature || typeof feature !== 'object'){ return true; }
    const template = typeof feature.template === 'string' ? feature.template : '';
    return template === 'profile-presets'
      || template === 'user-feedback'
      || template === 'accessibility-statement'
      || template === 'project-credits'
      || isDedicatedGlossaryFeature(feature)
      || isDedicatedStructureFeature(feature);
  }

  function collectFeatureSlugs(feature, target){
    if(!feature || !target){ return; }
    if(typeof feature.slug === 'string' && feature.slug && !isPassiveFeature(feature)){
      target.add(feature.slug);
    }
    if(Array.isArray(feature.children)){
      feature.children.forEach(child => collectFeatureSlugs(child, target));
    }
  }

  const sectionFeatureKeys = new Map();
  sectionsData.forEach(section => {
    if(!section || typeof section.id !== 'string' || !section.id){ return; }
    const keys = new Set();
    const features = Array.isArray(section.features) ? section.features : [];
    features.forEach(feature => collectFeatureSlugs(feature, keys));
    sectionFeatureKeys.set(section.id, keys);
  });

  function getActiveFeatureKeys(){
    return Object.keys(featureState).filter(key => !!featureState[key]);
  }

  function getActiveFeatureCount(){
    return getActiveFeatureKeys().length;
  }

  function formatActiveSummary(count){
    if(count <= 0){ return 'Aucun réglage actif'; }
    if(count === 1){ return '1 réglage actif'; }
    return `${count} réglages actifs`;
  }

  function syncFeatureCardState(card){
    if(!card){ return; }
    const inputs = Array.from(card.querySelectorAll('[data-role="feature-input"]'));
    const buttons = Array.from(card.querySelectorAll('[data-role="feature-toggle-button"]'));
    const hasActiveInput = inputs.some(input => !!input.checked);
    const hasActiveButton = buttons.some(button => button.getAttribute('aria-pressed') === 'true');
    card.classList.toggle('is-active', hasActiveInput || hasActiveButton);
  }

  function syncFeatureControlCardState(control){
    if(!control || typeof control.closest !== 'function'){ return; }
    syncFeatureCardState(control.closest('[data-role="feature-card"]'));
  }

  function syncAllRenderedCardStates(){
    if(!root){ return; }
    root.querySelectorAll('[data-role="feature-card"]').forEach(card => syncFeatureCardState(card));
  }

  function updateActiveSummary(){
    const activeKeys = getActiveFeatureKeys();
    const count = activeKeys.length;
    if(activeSummaryCount){ activeSummaryCount.textContent = String(count); }
    if(activeSummaryLabel){ activeSummaryLabel.textContent = formatActiveSummary(count); }
    if(activeSummary){
      activeSummary.classList.toggle('has-active', count > 0);
    }
    if(activeResetBtn){
      activeResetBtn.disabled = count === 0;
      activeResetBtn.setAttribute('aria-disabled', count === 0 ? 'true' : 'false');
    }
    sectionToggles.forEach(toggle => {
      const sectionId = toggle.dataset.sectionId || '';
      const keys = sectionFeatureKeys.get(sectionId) || new Set();
      let sectionCount = 0;
      keys.forEach(key => {
        if(featureState[key]){ sectionCount++; }
      });
      const countEl = toggle.querySelector('[data-role="section-active-count"]');
      if(countEl){
        countEl.hidden = sectionCount === 0;
        countEl.textContent = sectionCount === 1 ? '1 actif' : `${sectionCount} actifs`;
      }
      const labelEl = toggle.querySelector('.a11y-tab__label');
      const labelText = labelEl ? labelEl.textContent.trim() : '';
      toggle.classList.toggle('has-active', sectionCount > 0);
      if(labelText){
        toggle.setAttribute('aria-label', sectionCount > 0
          ? `${labelText}, ${sectionCount === 1 ? '1 réglage actif' : `${sectionCount} réglages actifs`}`
          : labelText);
      }
    });
    syncProfilePresetInstances();
    syncUndoButton();
  }

  function getFeatureLabelByKey(key){
    const entry = featureCatalog.get(key);
    if(entry && entry.feature && typeof entry.feature.label === 'string' && entry.feature.label){
      return entry.feature.label;
    }
    return key || '';
  }

  function syncUndoButton(){
    if(!activeUndoBtn){ return; }
    const hasUndo = !!(lastUndoAction && lastUndoAction.key);
    activeUndoBtn.disabled = !hasUndo;
    activeUndoBtn.setAttribute('aria-disabled', hasUndo ? 'false' : 'true');
    const label = hasUndo
      ? `Annuler le dernier changement : ${getFeatureLabelByKey(lastUndoAction.key)}`
      : 'Aucun changement à annuler';
    activeUndoBtn.setAttribute('aria-label', label);
    activeUndoBtn.setAttribute('title', label);
  }

  function queryRenderedFeatureControls(key){
    if(!key || !root){ return { inputs: [], buttons: [] }; }
    const inputs = Array.from(root.querySelectorAll('[data-role="feature-input"]')).filter(input => input.dataset.feature === key);
    const buttons = Array.from(root.querySelectorAll('[data-role="feature-toggle-button"]')).filter(button => button.dataset.feature === key);
    return { inputs, buttons };
  }

  function syncRenderedFeatureControls(key, on){
    const controls = queryRenderedFeatureControls(key);
    controls.inputs.forEach(input => {
      if(input.checked !== on){ input.checked = on; }
      syncFeatureControlCardState(input);
    });
    controls.buttons.forEach(button => {
      updateFeatureToggleButton(button, on);
    });
  }
  ...
  function updateFavoriteButton(button){
    ...
    const entry = featureCatalog.get(key);
    const label = entry && entry.feature ? (entry.feature.label || key) : key;
    const isFavorite = favoriteKeys.includes(key);
    const canAdd = favoriteKeys.length < FAVORITES_LIMIT;
    button.classList.toggle('is-active', isFavorite);
    button.setAttribute('aria-pressed', isFavorite ? 'true' : 'false');
    button.disabled = !isFavorite && !canAdd;
    button.setAttribute('aria-disabled', button.disabled ? 'true' : 'false');
    const action = isFavorite ? 'Retirer des favoris' : 'Ajouter aux favoris';
    const limitText = !isFavorite && !canAdd ? 'La limite de 5 favoris est atteinte.' : `${action} : ${label}`;
    button.setAttribute('aria-label', limitText);
    button.setAttribute('title', limitText);
  }

  function syncFavoriteButtons(){
    if(!root){ return; }
    root.querySelectorAll('[data-role="favorite-toggle"]').forEach(button => updateFavoriteButton(button));
  }

  function toggleFavorite(key){
    if(!key || !featureCatalog.has(key)){ return; }
    if(favoriteKeys.includes(key)){
      favoriteKeys = favoriteKeys.filter(item => item !== key);
    } else {
      if(favoriteKeys.length >= FAVORITES_LIMIT){ return; }
      favoriteKeys = favoriteKeys.concat(key).slice(0, FAVORITES_LIMIT);
    }
    persistFavoriteKeys();
    renderFavorites();
    syncFavoriteButtons();
  ...
  function createFavoriteToggle(feature){
    const key = feature && typeof feature.slug === 'string' ? feature.slug : '';
    if(!key || !featureCatalog.has(key)){ return null; }
    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'a11y-favorite-toggle';
    button.dataset.favoriteKey = key;
    button.setAttribute('data-role', 'favorite-toggle');
  ...
  function decorateFeatureCard(card, feature){
    if(!card || !feature || typeof feature.slug !== 'string'){ return card; }
    if(!featureCatalog.has(feature.slug)){ return card; }
    if(card.querySelector('[data-role="favorite-toggle"]')){ return card; }
    const favoriteButton = createFavoriteToggle(feature);
    if(!favoriteButton){ return card; }
    const meta = card.querySelector('[data-role="feature-meta"]') || card.querySelector('.meta');
    if(meta){
      meta.appendChild(favoriteButton);
    } else {
      card.insertBefore(favoriteButton, card.firstChild || null);
    }
    card.dataset.favoriteFeature = feature.slug;
    return card;
  }

  function decorateFeatureInstance(instance, feature){
    if(!instance){ return instance; }
    const card = instance instanceof DocumentFragment
      ? instance.querySelector('[data-role="feature-card"]')
      : instance;
    decorateFeatureCard(card, feature);
    return instance;
  }

  function createFavoriteInstance(entry){
    if(!entry || !entry.feature){ return null; }
    const feature = entry.feature;
    const instance = createCustomFeature(feature);
    const card = instance instanceof DocumentFragment
      ? instance.querySelector('[data-role="feature-card"]')
      : instance;
    if(card){
      card.classList.add('a11y-card--favorite');
      const meta = card.querySelector('[data-role="feature-meta"]') || card.querySelector('.meta');
      if(meta && entry.sectionTitle){
        const badge = document.createElement('span');
        badge.className = 'context';
        badge.textContent = entry.sectionTitle;
  ...
  function renderFavorites(){
    ...
    const validKeys = favoriteKeys.filter(key => featureCatalog.has(key)).slice(0, FAVORITES_LIMIT);
    if(validKeys.length !== favoriteKeys.length){
      favoriteKeys = validKeys;
      persistFavoriteKeys();
    }
    if(favoritesCount){
      favoritesCount.textContent = `${validKeys.length}/${FAVORITES_LIMIT}`;
    }
    favoritesPanel.hidden = !validKeys.length;
    if(favoritesEmpty){ favoritesEmpty.hidden = !!validKeys.length; }
    validKeys.forEach(key => {
      const entry = featureCatalog.get(key);
      const instance = createFavoriteInstance(entry);
      if(instance){ favoritesList.appendChild(instance); }
    });
    syncAllRenderedCardStates();
  }

  function undoLastFeatureChange(){
    if(!lastUndoAction || !lastUndoAction.key){ return; }
    const action = lastUndoAction;
    lastUndoAction = null;
    isApplyingUndo = true;
    toggleFeature(action.key, !!action.previous, { recordUndo: false });
    isApplyingUndo = false;
    persist();
    syncUndoButton();
  }

  function findFeatureInList(features, predicate){
    if(!Array.isArray(features)){ return null; }
    for(const feature of features){
      if(!feature){ continue; }
      if(predicate(feature)){ return feature; }
      if(Array.isArray(feature.children)){
        const childMatch = findFeatureInList(feature.children, predicate);
  ...
  function getDedicatedGlossaryFeature(){
    for(const section of sectionsData){
      if(!section){ continue; }
      const match = findFeatureInList(section.features, isDedicatedGlossaryFeature);
      if(match){ return match; }
    }
    return null;
  }

  function getDedicatedStructureFeature(){
    for(const section of sectionsData){
      if(!section){ continue; }
      const match = findFeatureInList(section.features, isDedicatedStructureFeature);
      if(match){ return match; }
    }
    return null;
  }
  ...
  function collectShortcutCandidates(feature){
    ...
    if(isDedicatedGlossaryFeature(feature)){ return; }
    if(isDedicatedStructureFeature(feature)){ return; }
  ...
  function populateTutorialList(){
    ...
    if(!shortcutDisplayList.length){
      ...
      if(tutorialToggle){
        ...
        tutorialToggle.classList.remove('is-active');
      }
      if(tutorialSection){
        tutorialSection.hidden = true;
        tutorialSection.setAttribute('aria-hidden', 'true');
      }
      closeShortcutsDialog({ restoreFocus: false, immediate: true });
      return;
    }
  ...
  function setHeaderViewButtonState(button, active){
    if(!button){ return; }
    button.classList.toggle('is-active', !!active);
    button.setAttribute('aria-pressed', active ? 'true' : 'false');
  }

  function populateInfoGlossary(){
    if(!infoGlossaryContent || infoGlossaryRendered){ return; }
    const feature = getDedicatedGlossaryFeature();
    if(!feature){
      if(infoGlossaryEmpty){ infoGlossaryEmpty.hidden = false; }
      return;
    }
    const card = createGlossaryCard(feature);
    if(card){
      infoGlossaryContent.appendChild(card);
      infoGlossaryRendered = true;
      if(infoGlossaryEmpty){ infoGlossaryEmpty.hidden = true; }
      return;
    }
    if(infoGlossaryEmpty){ infoGlossaryEmpty.hidden = false; }
  }

  function populateStructureView(){
    if(!structureViewContent || structureViewRendered){ return; }
    const feature = getDedicatedStructureFeature();
    if(!feature){
      if(structureViewEmpty){ structureViewEmpty.hidden = false; }
      return;
    }
    const card = createPageStructureCard(feature);
    if(card){
      structureViewContent.appendChild(card);
      structureViewRendered = true;
      if(structureViewEmpty){ structureViewEmpty.hidden = true; }
      return;
    }
    if(structureViewEmpty){ structureViewEmpty.hidden = false; }
  }

  function setPanelView(view, opts={}){
    const requestedStructure = view === 'structure';
    const canShowStructure = !!(structureView && structureToggle);
    const showStructure = requestedStructure && canShowStructure;
    const showOptions = !showStructure;

    setHeaderViewButtonState(optionsToggle, showOptions);
    setHeaderViewButtonState(structureToggle, showStructure);

    if(structureToggle){
      structureToggle.setAttribute('aria-expanded', showStructure ? 'true' : 'false');
    }
    if(structureView){
      structureView.hidden = !showStructure;
      structureView.setAttribute('aria-hidden', showStructure ? 'false' : 'true');
    }
    if(optionsView){
      optionsView.hidden = !showOptions;
      optionsView.setAttribute('aria-hidden', showOptions ? 'false' : 'true');
    }
    if(panel){
      panel.dataset.activeView = showStructure ? 'structure' : 'options';
    }
    if(showStructure && !opts.skipPopulate){
      populateStructureView();
    }
  }

  function closeStructureView(options = {}){
    const { restoreFocus = true } = options;
    setPanelView('options');
    if(restoreFocus && structureToggle && typeof structureToggle.focus === 'function'){
      try { structureToggle.focus({ preventScroll: true }); } catch(err){ structureToggle.focus(); }
    }
  }
  ...
  function handleShortcutKeydown(event){
    ...
    const toggleButton = featureToggleButtons.get(entry.slug);
    const focusTarget = input || toggleButton;
  ...
  sectionToggles.forEach(toggle => {
    const sectionId = toggle.dataset.sectionId || '';
    if(!sectionId){ return; }
    const container = toggle.closest('[data-role="accordion-item"]');
    if(!container){ return; }
  ...
  const EPILEPSY_TEXT_DEFAULTS = {
    ...
    block_flashing_hint: '',
    activate_all_label: 'Activer toutes les réductions',
    activate_all_aria: '',
    activate_all_confirm: '',
    reset_label: 'Réinitialiser',
    reset_aria: '',
    live_region_label: '',
    gif_placeholder_label: 'GIF animé désactivé',
    gif_placeholder_hint: '',
    flash_overlay_title: 'Variation lumineuse détectée',
    flash_overlay_body: 'Une variation lumineuse a été détectée. La page est masquée temporairement pour réduire l’exposition à un déclencheur visuel potentiel.',
  ...
  function showEpilepsyFlashOverlay(){
    ...
    title.id = titleId;
    title.textContent = epilepsyTexts.flash_overlay_title || 'Variation lumineuse détectée';
  ...
  function handleEpilepsyFlashDetected(){
    ...
    const message = epilepsyTexts.flash_overlay_title || 'Variation lumineuse détectée';
  ...
  function finalizeEpilepsySettingsChange(options = {}){
    if(options.persist !== false){
      persistEpilepsySettings();
    } else {
      try { localStorage.removeItem(EPILEPSY_SETTINGS_KEY); } catch(err){ /* ignore */ }
    }
    applyEpilepsySettings();
    syncEpilepsyInstances();
  }

  function resetEpilepsySettings(options = {}){
    ...
    if(changed){
      finalizeEpilepsySettingsChange(options);
    } else {
      if(options.persist === false){
        try { localStorage.removeItem(EPILEPSY_SETTINGS_KEY); } catch(err){ /* ignore */ }
      }
  ...
  function createEpilepsyCard(feature){
    ...
    resetBtn.addEventListener('click', () => {
      ...
      if(changed){
        const message = texts.reset_label || texts.reset_aria || 'Réinitialisation des réductions de déclencheurs visuels';
        announceEpilepsy(message);
      }
    });
  ...
  function resetBrailleSettings(options = {}){
    brailleSettings = getDefaultBrailleSettings();
    brailleSelectionState = { text: '', truncated: false };
    brailleInstances.forEach(set => {
      if(set instanceof Set){
        set.forEach(instance => closeDetachedBrailleViewer(instance));
      }
    });
    if(options.persist !== false){
      persistBrailleSettings();
    } else {
      try { localStorage.removeItem(BRAILLE_SETTINGS_KEY); } catch(err){ /* ignore */ }
    }
  ...
  function syncSingleBrailleInstance(instance, state){
    ...
    const viewerExample = texts.viewer_example || 'MOBLS Montrer la visionneuse Braille au démarrage';
    const viewerSource = hasResult ? state.lastOriginal : viewerExample;
    const viewerBrailleText = hasResult ? state.lastBraille : translateBrailleText(viewerSource, instance.mode);
    if(viewerOriginal){
      viewerOriginal.textContent = viewerSource;
    }
    if(viewerCells){
      buildBrailleCells(viewerCells, viewerBrailleText);
    }
    if(viewerMode){
      const modeLabel = texts.viewer_mode_label || 'Mode affiché';
      const modeValue = texts.viewer_mode_value || (instance.mode === 'contracted' ? 'Braille contracté' : 'Braille non contracté');
      instance.viewerModeText = `${modeLabel} : ${modeValue}`;
      viewerMode.textContent = instance.viewerModeText;
    }
    instance.viewerSourceText = viewerSource;
    instance.viewerBrailleText = viewerBrailleText;
    syncDetachedBrailleViewer(instance);
  }
  ...
  function createBrailleViewerOption(label, checked=false){
    const option = document.createElement('span');
    option.className = 'a11y-braille-viewer__option';
    const checkbox = document.createElement('span');
    checkbox.className = 'a11y-braille-viewer__checkbox';
    checkbox.setAttribute('aria-hidden', 'true');
    if(checked){ checkbox.classList.add('is-checked'); }
    option.appendChild(checkbox);
    const text = document.createElement('span');
    text.className = 'a11y-braille-viewer__option-text';
  ...
  function setBrailleViewerStatus(instance, text=''){
    if(!instance || !instance.viewerStatus){ return; }
    instance.viewerStatus.textContent = text || '';
  }

  function syncDetachedBrailleViewer(instance){
    if(!instance || !instance.detachedViewer){ return; }
    const detached = instance.detachedViewer;
    if(detached.cells){ buildBrailleCells(detached.cells, instance.viewerBrailleText || ''); }
    if(detached.original){ detached.original.textContent = instance.viewerSourceText || ''; }
    if(detached.mode){ detached.mode.textContent = instance.viewerModeText || ''; }
  }

  function closeDetachedBrailleViewer(instance){
    if(!instance || !instance.detachedViewer){ return; }
    const { wrapper } = instance.detachedViewer;
    if(wrapper && wrapper.parentNode){
      wrapper.parentNode.removeChild(wrapper);
    }
    instance.detachedViewer = null;
  }

  function startDetachedBrailleViewerDrag(wrapper, event){
    if(!wrapper || !event || (event.pointerType === 'mouse' && event.button !== 0)){ return; }
    ...
    const stop = stopEvent => {
      ...
      if(wrapper.releasePointerCapture){
        try { wrapper.releasePointerCapture(stopEvent.pointerId); } catch(err){}
      }
    };
  ...
  function openDetachedBrailleViewer(instance){
    if(!instance){ return; }
    if(instance.detachedViewer && instance.detachedViewer.wrapper && instance.detachedViewer.wrapper.isConnected){
      try { instance.detachedViewer.wrapper.focus({ preventScroll: true }); } catch(err){ instance.detachedViewer.wrapper.focus(); }
      return;
    }
    const wrapper = document.createElement('div');
    wrapper.className = 'a11y-braille-viewer-popout';
    wrapper.setAttribute('tabindex', '-1');
    wrapper.setAttribute('role', 'dialog');
    wrapper.setAttribute('aria-label', instance.texts.viewer_title || 'Visionneuse Braille de MOBLS');

    const viewer = document.createElement('div');
    viewer.className = 'a11y-braille-viewer a11y-braille-viewer--detached';
    const titlebar = document.createElement('div');
    titlebar.className = 'a11y-braille-viewer__titlebar';
    titlebar.addEventListener('pointerdown', event => startDetachedBrailleViewerDrag(wrapper, event));

    const icon = document.createElement('span');
    icon.className = 'a11y-braille-viewer__icon';
    icon.setAttribute('aria-hidden', 'true');
    titlebar.appendChild(icon);
    const title = document.createElement('span');
    title.className = 'a11y-braille-viewer__title';
    title.textContent = instance.texts.viewer_title || 'Visionneuse Braille de MOBLS';
    titlebar.appendChild(title);
    const closeButton = document.createElement('button');
    closeButton.type = 'button';
    closeButton.className = 'a11y-braille-viewer__tool';
    closeButton.setAttribute('aria-label', instance.texts.viewer_close_label || 'Fermer la visionneuse Braille détachée');
    closeButton.textContent = '×';
    closeButton.addEventListener('click', () => closeDetachedBrailleViewer(instance));
    titlebar.appendChild(closeButton);
    viewer.appendChild(titlebar);

    const display = document.createElement('div');
    display.className = 'a11y-braille-viewer__display';
    const cells = document.createElement('div');
    cells.className = 'a11y-braille-viewer__cells';
    cells.setAttribute('aria-hidden', 'true');
    display.appendChild(cells);
    const original = document.createElement('div');
    original.className = 'a11y-braille-viewer__text';
    display.appendChild(original);
    viewer.appendChild(display);

    const footer = document.createElement('div');
    footer.className = 'a11y-braille-viewer__footer';
    const mode = document.createElement('p');
    mode.className = 'a11y-braille-viewer__mode';
    footer.appendChild(mode);
    viewer.appendChild(footer);

    wrapper.appendChild(viewer);
    (root || document.body).appendChild(wrapper);
    instance.detachedViewer = { wrapper, cells, original, mode };
    syncDetachedBrailleViewer(instance);
    try { wrapper.focus({ preventScroll: true }); } catch(err){ wrapper.focus(); }
  }
  ...
  function createBrailleCard(feature){
    ...
    const texts = {
      ...
      viewer_title: typeof settings.viewer_title === 'string' ? settings.viewer_title : 'Visionneuse Braille de MOBLS',
      viewer_aria: typeof settings.viewer_aria === 'string' ? settings.viewer_aria : 'Exemple visuel de visionneuse braille',
      viewer_example: typeof settings.viewer_example === 'string' ? settings.viewer_example : 'MOBLS Montrer la visionneuse Braille au démarrage',
      viewer_mode_label: typeof settings.viewer_mode_label === 'string' ? settings.viewer_mode_label : 'Mode affiché',
      viewer_mode_value: typeof settings.viewer_mode_value === 'string' ? settings.viewer_mode_value : '',
      viewer_startup_label: typeof settings.viewer_startup_label === 'string' ? settings.viewer_startup_label : 'Montrer la visionneuse Braille au démarrage',
      viewer_follow_label: typeof settings.viewer_follow_label === 'string' ? settings.viewer_follow_label : 'Suivi de la sélection vers une cellule',
      viewer_copy_label: typeof settings.viewer_copy_label === 'string' ? settings.viewer_copy_label : 'Copier la transcription braille',
      viewer_copied_label: typeof settings.viewer_copied_label === 'string' ? settings.viewer_copied_label : 'Transcription braille copiée.',
      viewer_large_label: typeof settings.viewer_large_label === 'string' ? settings.viewer_large_label : 'Agrandir les points braille',
      viewer_normal_label: typeof settings.viewer_normal_label === 'string' ? settings.viewer_normal_label : 'Réduire les points braille',
      viewer_detach_label: typeof settings.viewer_detach_label === 'string' ? settings.viewer_detach_label : 'Détacher la visionneuse Braille',
      viewer_close_label: typeof settings.viewer_close_label === 'string' ? settings.viewer_close_label : 'Fermer la visionneuse Braille détachée',
    };
    ...
    viewerSection.className = 'a11y-braille__section a11y-braille__section--viewer';
    const viewerTitle = document.createElement('p');
    viewerTitle.className = 'a11y-braille__title';
    viewerTitle.textContent = texts.viewer_title || 'Visionneuse Braille de MOBLS';
    viewerSection.appendChild(viewerTitle);

    const viewerWindow = document.createElement('div');
    viewerWindow.className = 'a11y-braille-viewer';
    viewerWindow.setAttribute('role', 'group');
    viewerWindow.setAttribute('aria-label', texts.viewer_aria || 'Exemple visuel de visionneuse braille');

    const viewerBar = document.createElement('div');
    viewerBar.className = 'a11y-braille-viewer__titlebar';
    const viewerIcon = document.createElement('span');
    viewerIcon.className = 'a11y-braille-viewer__icon';
    viewerIcon.setAttribute('aria-hidden', 'true');
    viewerBar.appendChild(viewerIcon);
    const viewerBarTitle = document.createElement('span');
    viewerBarTitle.className = 'a11y-braille-viewer__title';
    viewerBarTitle.textContent = texts.viewer_title || 'Visionneuse Braille de MOBLS';
    viewerBar.appendChild(viewerBarTitle);
    const viewerTools = document.createElement('span');
    viewerTools.className = 'a11y-braille-viewer__tools';
    const viewerCopy = document.createElement('button');
    viewerCopy.type = 'button';
    viewerCopy.className = 'a11y-braille-viewer__tool';
    viewerCopy.setAttribute('aria-label', texts.viewer_copy_label || 'Copier la transcription braille');
    viewerCopy.textContent = 'Copier';
    viewerTools.appendChild(viewerCopy);
    const viewerLarge = document.createElement('button');
    viewerLarge.type = 'button';
    viewerLarge.className = 'a11y-braille-viewer__tool';
    viewerLarge.setAttribute('aria-pressed', 'false');
    viewerLarge.setAttribute('aria-label', texts.viewer_large_label || 'Agrandir les points braille');
    viewerLarge.textContent = 'Agrandir';
    viewerTools.appendChild(viewerLarge);
    const viewerDetach = document.createElement('button');
    viewerDetach.type = 'button';
    viewerDetach.className = 'a11y-braille-viewer__tool';
    viewerDetach.setAttribute('aria-label', texts.viewer_detach_label || 'Détacher la visionneuse Braille');
    viewerDetach.textContent = 'Détacher';
    viewerTools.appendChild(viewerDetach);
    viewerBar.appendChild(viewerTools);
    const viewerClose = document.createElement('span');
    viewerClose.className = 'a11y-braille-viewer__close';
    viewerClose.setAttribute('aria-hidden', 'true');
    viewerClose.textContent = '×';
    viewerBar.appendChild(viewerClose);
    viewerWindow.appendChild(viewerBar);

    const viewerDisplay = document.createElement('div');
    viewerDisplay.className = 'a11y-braille-viewer__display';
    const viewerCells = document.createElement('div');
    viewerCells.className = 'a11y-braille-viewer__cells';
    viewerCells.setAttribute('aria-hidden', 'true');
    viewerDisplay.appendChild(viewerCells);
    const viewerOriginal = document.createElement('div');
    viewerOriginal.className = 'a11y-braille-viewer__text';
    viewerDisplay.appendChild(viewerOriginal);
    viewerWindow.appendChild(viewerDisplay);

    const viewerFooter = document.createElement('div');
    viewerFooter.className = 'a11y-braille-viewer__footer';
    const viewerMode = document.createElement('p');
    viewerMode.className = 'a11y-braille-viewer__mode';
    viewerFooter.appendChild(viewerMode);
    const viewerStatus = document.createElement('p');
    viewerStatus.className = 'a11y-braille-viewer__status';
    viewerStatus.setAttribute('role', 'status');
    viewerStatus.setAttribute('aria-live', 'polite');
    viewerFooter.appendChild(viewerStatus);
    const viewerOptions = document.createElement('div');
    viewerOptions.className = 'a11y-braille-viewer__options';
    viewerOptions.appendChild(createBrailleViewerOption(texts.viewer_startup_label, false));
    viewerOptions.appendChild(createBrailleViewerOption(texts.viewer_follow_label, false));
    viewerFooter.appendChild(viewerOptions);
    viewerWindow.appendChild(viewerFooter);

    viewerSection.appendChild(viewerWindow);
    article.appendChild(viewerSection);
    ...
    viewerCopy.addEventListener('click', () => {
      copyTextToClipboard(instance.viewerBrailleText || '').then(copied => {
        setBrailleViewerStatus(instance, copied ? (texts.viewer_copied_label || 'Transcription braille copiée.') : '');
      });
    });
    viewerLarge.addEventListener('click', () => {
      const next = viewerLarge.getAttribute('aria-pressed') !== 'true';
      viewerLarge.setAttribute('aria-pressed', next ? 'true' : 'false');
      viewerLarge.setAttribute('aria-label', next
        ? (texts.viewer_normal_label || 'Réduire les points braille')
        : (texts.viewer_large_label || 'Agrandir les points braille'));
      viewerLarge.textContent = next ? 'Réduire' : 'Agrandir';
      viewerWindow.classList.toggle('is-large-dots', next);
    });
    viewerDetach.addEventListener('click', () => {
      openDetachedBrailleViewer(instance);
    });
  ...
  function resetTtsSettings(options = {}){
    if(ttsSupport && ttsSynth){
      try { ttsSynth.cancel(); } catch(err){ /* ignore */ }
    }
    resetTtsPlaybackState();
    ttsSettings = Object.assign({}, TTS_DEFAULTS);
    ttsSelectionText = '';
    if(options.persist !== false){
      persistTtsSettings();
    } else {
      try { localStorage.removeItem(TTS_STORAGE_KEY); } catch(err){ /* ignore */ }
    }
    ensureTtsSelectionTracking();
  ...
  const DYSLEXIA_LETTER_SPACING_MIN = 0;
  const DYSLEXIA_LETTER_SPACING_MAX = 0.12;
  const DYSLEXIA_LETTER_SPACING_STEP = 0.01;
  const DYSLEXIA_WORD_SPACING_MIN = 0;
  const DYSLEXIA_WORD_SPACING_MAX = 0.24;
  const DYSLEXIA_WORD_SPACING_STEP = 0.02;
  const DYSLEXIA_PARAGRAPH_SPACING_MIN = 0;
  const DYSLEXIA_PARAGRAPH_SPACING_MAX = 2;
  const DYSLEXIA_PARAGRAPH_SPACING_STEP = 0.1;
  const DYSLEXIA_LINE_LENGTH_MIN = 52;
  const DYSLEXIA_LINE_LENGTH_MAX = 84;
  const DYSLEXIA_LINE_LENGTH_STEP = 2;
  ...
  const DYSLEXIA_FLOW_SELECTOR = 'body :where(p, li, blockquote, dd, dt, figcaption, h1, h2, h3, h4, h5, h6):not(#a11y-widget-root *)';
  const DYSLEXIA_BLOCK_SELECTOR = 'body :where(p, li, blockquote, dd, dt):not(#a11y-widget-root *)';
  ...
  const DYSLEXIA_PATTERN_PRESETS = {
    none: [],
    graphemes: ['eau', 'ain', 'ein', 'ill', 'ou', 'on', 'an', 'en', 'in', 'au', 'ch', 'gn', 'ph', 'qu'],
    confusions: ['b', 'd', 'p', 'q', 'm', 'n', 'u', 'i', 'l', '1'],
    morphemes: ['tion', 'sion', 'ment', 'age', 'able', 'ible', 'eur', 'euse', 'ette', 'isme', 'anti', 'auto', 'pre', 'pré', 'dé']
  };
  const DYSLEXIA_LAYOUT_PRESETS = {
    soft: {
      font: 'verdana',
      fontSize: 18,
      lineHeight: 160,
  ...
  const READING_GUIDE_MODE_PRESETS = {
    bar: { mode: 'bar', color: '#ffe28a', opacity: 0.65, height: 120 },
    lightbox: { mode: 'lightbox', color: '#fff7c2', opacity: 0.92, height: 96 },
    shade: { mode: 'shade', color: '#d1d5db', opacity: 0.52, height: 128 },
    underline: { mode: 'underline', color: '#2563eb', opacity: 0.95, height: 72 },
  };
  ...
  let readingGuideControlIdCounter = 0;
  ...
  const featureToggleButtons = new Map();
  ...
  function updateSideToggleUI(side){
    ...
    const label = targetSide === 'right'
      ? ((sideToggleLabels && sideToggleLabels.right) || '')
      : ((sideToggleLabels && sideToggleLabels.left) || '');
    const text = targetSide === 'right'
      ? ((sideToggleTexts && sideToggleTexts.right) || 'À droite')
      : ((sideToggleTexts && sideToggleTexts.left) || 'À gauche');
    const status = side === 'left'
      ? ((sideToggleStatuses && sideToggleStatuses.left) || 'Le panneau est placé à gauche.')
      : ((sideToggleStatuses && sideToggleStatuses.right) || 'Le panneau est placé à droite.');
    if(label){
      sideToggleBtn.setAttribute('aria-label', label);
      sideToggleBtn.setAttribute('title', label);
      sideToggleBtn.dataset.tooltip = label;
    }
    sideToggleBtn.dataset.panelSide = side === 'left' ? 'left' : 'right';
    sideToggleBtn.dataset.panelTarget = targetSide;
    if(sideToggleText){ sideToggleText.textContent = text; }
    if(positionStatus){ positionStatus.textContent = status; }
  }
  ...
  function applyPanelSide(side){
    ...
    if(shortcutsDialogIsOpen){
      updateShortcutsDialogPosition();
    }
    if(infoDialog){
  ...
  function clearFeatureGrid(targetGrid){
    ...
    const toggleButtons = targetGrid.querySelectorAll('[data-role="feature-toggle-button"]');
    toggleButtons.forEach(button => {
      const key = button.dataset.feature;
      if(key){ featureToggleButtons.delete(key); }
    });
  ...
  function updateFeatureToggleButton(button, active){
    if(!button){ return; }
    const isActive = !!active;
    button.setAttribute('aria-pressed', isActive ? 'true' : 'false');
    button.textContent = isActive ? 'Activé' : 'Activer';
    syncFeatureControlCardState(button);
  }

  function registerFeatureToggleButton(key, button){
    if(!key || !button){ return; }
    if(featureToggleButtons.has(key) && featureToggleButtons.get(key) !== button){
      featureToggleButtons.delete(key);
    }
    featureToggleButtons.set(key, button);
    const stored = Object.prototype.hasOwnProperty.call(featureState, key) ? !!featureState[key] : false;
    updateFeatureToggleButton(button, stored);
  ...
  function createFeaturePlaceholder(feature){
    ...
    const article = document.createElement('article');
    article.className = 'a11y-card a11y-card--standard-toggle';
    article.setAttribute('data-role', 'feature-card');

    const metaEl = document.createElement('div');
    metaEl.className = 'meta';
    metaEl.setAttribute('data-role', 'feature-meta');
    article.appendChild(metaEl);

    const labelEl = document.createElement('span');
    labelEl.className = 'label';
    labelEl.setAttribute('data-role', 'feature-label');
    labelEl.textContent = feature.label || '';
    ...
    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'a11y-standard-toggle';
    button.dataset.feature = slug;
    button.setAttribute('data-role', 'feature-toggle-button');
    button.setAttribute('aria-label', feature.label || feature.aria_label || slug);
    applyShortcutToInput(slug, button, feature.label || feature.aria_label || '');
    article.appendChild(button);
    registerFeatureToggleButton(slug, button);

    return decorateFeatureCard(article, feature);
  }
  ...
  function createColorblindCard(feature){
    ...
    children.forEach(child => {
      ...
      const favoriteButton = createFavoriteToggle(child);
      if(favoriteButton){
        labelWrapper.appendChild(favoriteButton);
      }
  ...
  function createFeatureGroup(feature){
    ...
    children.forEach(child => {
      ...
      const favoriteButton = createFavoriteToggle(child);
      if(favoriteButton){
        rowMeta.appendChild(favoriteButton);
      }
  ...
  function clampDyslexiaDecimal(value, min, max, step, fallback){
    const numeric = typeof value === 'number' ? value : parseFloat(value);
  ...
  function clampDyslexiaLetterSpacing(value){
    return clampDyslexiaDecimal(value, DYSLEXIA_LETTER_SPACING_MIN, DYSLEXIA_LETTER_SPACING_MAX, DYSLEXIA_LETTER_SPACING_STEP, DYSLEXIA_DEFAULTS.letterSpacing);
  }

  function clampDyslexiaWordSpacing(value){
    return clampDyslexiaDecimal(value, DYSLEXIA_WORD_SPACING_MIN, DYSLEXIA_WORD_SPACING_MAX, DYSLEXIA_WORD_SPACING_STEP, DYSLEXIA_DEFAULTS.wordSpacing);
  }

  function clampDyslexiaParagraphSpacing(value){
    return clampDyslexiaDecimal(value, DYSLEXIA_PARAGRAPH_SPACING_MIN, DYSLEXIA_PARAGRAPH_SPACING_MAX, DYSLEXIA_PARAGRAPH_SPACING_STEP, DYSLEXIA_DEFAULTS.paragraphSpacing);
  }

  function clampDyslexiaLineLength(value){
    const numeric = typeof value === 'number' ? value : parseFloat(value);
    if(!isFinite(numeric)){ return DYSLEXIA_DEFAULTS.lineLength; }
    const rounded = Math.round(numeric / DYSLEXIA_LINE_LENGTH_STEP) * DYSLEXIA_LINE_LENGTH_STEP;
    return Math.min(DYSLEXIA_LINE_LENGTH_MAX, Math.max(DYSLEXIA_LINE_LENGTH_MIN, rounded));
  }

  function normalizeDyslexiaPatternPreset(value){
    if(typeof value !== 'string'){ return DYSLEXIA_DEFAULTS.patternPreset; }
    const key = value.trim().toLowerCase();
    return Object.prototype.hasOwnProperty.call(DYSLEXIA_PATTERN_PRESETS, key)
      ? key
      : DYSLEXIA_DEFAULTS.patternPreset;
  }
  ...
  function formatDyslexiaEm(value){
    return `${Number(value).toFixed(2).replace(/\.?0+$/, '')}em`;
  }

  function formatDyslexiaLetterSpacing(value){
    return formatDyslexiaEm(clampDyslexiaLetterSpacing(value));
  }

  function formatDyslexiaWordSpacing(value){
    return formatDyslexiaEm(clampDyslexiaWordSpacing(value));
  }

  function formatDyslexiaParagraphSpacing(value){
    return formatDyslexiaEm(clampDyslexiaParagraphSpacing(value));
  }

  function formatDyslexiaLineLength(value){
    return `${clampDyslexiaLineLength(value)}ch`;
  }
  ...
  function updateDyslexiaStyles(){
    ...
    const letterSpacing = clampDyslexiaLetterSpacing(dyslexiaSettings.letterSpacing);
    const wordSpacing = clampDyslexiaWordSpacing(dyslexiaSettings.wordSpacing);
    const paragraphSpacing = clampDyslexiaParagraphSpacing(dyslexiaSettings.paragraphSpacing);
    const lineLength = clampDyslexiaLineLength(dyslexiaSettings.lineLength);
    ...
    if(letterSpacing > 0 || wordSpacing > 0){
      rules.push(`${DYSLEXIA_FLOW_SELECTOR} { letter-spacing: ${letterSpacing}em !important; word-spacing: ${wordSpacing}em !important; }`);
    }
    if(paragraphSpacing > 0){
      rules.push(`${DYSLEXIA_BLOCK_SELECTOR} { margin-block-end: ${paragraphSpacing}em !important; }`);
    }
    if(lineLength !== DYSLEXIA_DEFAULTS.lineLength || dyslexiaSettings.alignLeft){
      rules.push(`${DYSLEXIA_BLOCK_SELECTOR} { max-width: ${lineLength}ch !important; }`);
    }
    if(dyslexiaSettings.alignLeft){
      rules.push(`${DYSLEXIA_FLOW_SELECTOR} { text-align: left !important; hyphens: manual !important; }`);
    }
    if(dyslexiaSettings.removePatterns){
      rules.push('body *:not(#a11y-widget-root):not(#a11y-widget-root *):not(#wpadminbar):not(#wpadminbar *) { background-image: none !important; text-shadow: none !important; }');
    }
    if(fontKey !== 'default'){
...
`; }
  ...
  function resetMigraineSettings(options = {}){
    migraineSettings = getDefaultMigraineSettings();
    applyMigraineSettings();
    if(options.persist !== false){
      persistMigraineSettings();
    } else {
      try { localStorage.removeItem(MIGRAINE_SETTINGS_KEY); } catch(err){ /* ignore */ }
    }
    syncMigraineInstances();
  }
  ...
  function loadDyslexiaSettings(){
    ...
    try {
      ...
      if(Object.prototype.hasOwnProperty.call(parsed, 'patternPreset')){
        result.patternPreset = normalizeDyslexiaPatternPreset(parsed.patternPreset);
      }
      ...
      if(Object.prototype.hasOwnProperty.call(parsed, 'letterSpacing')){
        result.letterSpacing = clampDyslexiaLetterSpacing(parsed.letterSpacing);
      }
      if(Object.prototype.hasOwnProperty.call(parsed, 'wordSpacing')){
        result.wordSpacing = clampDyslexiaWordSpacing(parsed.wordSpacing);
      }
      if(Object.prototype.hasOwnProperty.call(parsed, 'paragraphSpacing')){
        result.paragraphSpacing = clampDyslexiaParagraphSpacing(parsed.paragraphSpacing);
      }
      if(Object.prototype.hasOwnProperty.call(parsed, 'lineLength')){
        result.lineLength = clampDyslexiaLineLength(parsed.lineLength);
      }
      if(Object.prototype.hasOwnProperty.call(parsed, 'alignLeft')){
        result.alignLeft = !!parsed.alignLeft;
      }
      if(Object.prototype.hasOwnProperty.call(parsed, 'removePatterns')){
        result.removePatterns = !!parsed.removePatterns;
      }
  ...
  function persistDyslexiaSettings(){
    const payload = {
      ...
      patternPreset: normalizeDyslexiaPatternPreset(dyslexiaSettings.patternPreset),
      ...
      letterSpacing: clampDyslexiaLetterSpacing(dyslexiaSettings.letterSpacing),
      wordSpacing: clampDyslexiaWordSpacing(dyslexiaSettings.wordSpacing),
      paragraphSpacing: clampDyslexiaParagraphSpacing(dyslexiaSettings.paragraphSpacing),
      lineLength: clampDyslexiaLineLength(dyslexiaSettings.lineLength),
      alignLeft: !!dyslexiaSettings.alignLeft,
      removePatterns: !!dyslexiaSettings.removePatterns,
  ...
  function resetDyslexiaSettings(options = {}){
    dyslexiaSettings = Object.assign({}, DYSLEXIA_DEFAULTS);
    clearDyslexiaHighlights();
    updateDyslexiaStyles();
    if(options.persist !== false){
      persistDyslexiaSettings();
    } else {
      try { localStorage.removeItem(DYSLEXIA_SETTINGS_KEY); } catch(err){ /* ignore */ }
    }
  ...
  function normalizeReadingGuideMode(value){
    if(typeof value !== 'string'){ return READING_GUIDE_DEFAULTS.mode; }
    const key = value.trim().toLowerCase();
    return Object.prototype.hasOwnProperty.call(READING_GUIDE_MODE_PRESETS, key)
      ? key
      : READING_GUIDE_DEFAULTS.mode;
  }
  ...
  function loadReadingGuideSettings(){
    ...
    try {
      ...
      if(Object.prototype.hasOwnProperty.call(parsed, 'mode')){
        result.mode = normalizeReadingGuideMode(parsed.mode);
      }
      if(Object.prototype.hasOwnProperty.call(parsed, 'color')){
  ...
  function persistReadingGuideSettings(){
    const payload = {
      mode: normalizeReadingGuideMode(readingGuideSettings.mode),
  ...
  function applyReadingGuideStyles(){
    const rootEl = document.documentElement;
    if(!rootEl){ return; }
    const mode = normalizeReadingGuideMode(readingGuideSettings.mode);
    ...
    rootEl.dataset.a11yReadingGuideMode = mode;
    ...
    if(readingGuideOverlayEl){
      readingGuideOverlayEl.dataset.mode = mode;
  ...
  function applyReadingGuideModePreset(mode, options = {}){
    const key = normalizeReadingGuideMode(mode);
    const preset = READING_GUIDE_MODE_PRESETS[key];
    if(!preset){ return; }
    readingGuideSettings.mode = normalizeReadingGuideMode(preset.mode);
    readingGuideSettings.color = normalizeReadingGuideColor(preset.color);
    readingGuideSettings.opacity = clampReadingGuideOpacity(preset.opacity);
    readingGuideSettings.height = clampReadingGuideHeight(preset.height);
    if(options.persist !== false){ persistReadingGuideSettings(); }
    if(readingGuideActive){
      applyReadingGuideStyles();
      refreshReadingGuideOverlay();
    }
  ...
  function setReadingGuideSetting(key, value, options = {}){
    ...
    switch(key){
      case 'mode':
        next = normalizeReadingGuideMode(value);
        break;
      case 'color':
  ...
  function updateReadingGuideInstanceUI(instance){
    ...
    if(Array.isArray(modeButtons)){
      modeButtons.forEach(entry => {
        if(!entry || !entry.button){ return; }
        entry.button.disabled = !active;
        entry.button.setAttribute('aria-pressed', entry.mode === mode ? 'true' : 'false');
        entry.button.classList.toggle('is-active', entry.mode === mode);
      });
    }
    if(colorInput){
  ...
  function setReadingGuideActive(on){
    ...
    if(desired){
      ...
      if(document.documentElement){
        ...
        delete document.documentElement.dataset.a11yReadingGuideMode;
      }
    }
    syncReadingGuideInstances();
  }
  ...
  function createReadingGuideCard(feature){
    if(!feature || typeof feature.slug !== 'string' || !feature.slug){ return null; }
    const baseId = `a11y-reading-guide-${++readingGuideControlIdCounter}`;
    ...
    const modeButtons = [];
    const modeGroup = document.createElement('div');
    modeGroup.className = 'a11y-reading-guide__mode-group';
    if(settings.mode_label){
      const modeLabel = document.createElement('p');
      modeLabel.className = 'a11y-reading-guide__subheading';
      modeLabel.textContent = settings.mode_label;
      modeGroup.appendChild(modeLabel);
    }
    const modeGrid = document.createElement('div');
    modeGrid.className = 'a11y-reading-guide__mode-grid';
    [
      { mode: 'bar', label: settings.mode_bar_label || 'Barre' },
      { mode: 'lightbox', label: settings.mode_lightbox_label || 'Focus' },
      { mode: 'shade', label: settings.mode_shade_label || 'Ombre' },
      { mode: 'underline', label: settings.mode_underline_label || 'Soulignement' },
    ].forEach(entry => {
      const button = document.createElement('button');
      button.type = 'button';
      button.className = 'a11y-reading-guide__mode';
      button.dataset.mode = entry.mode;
      button.textContent = entry.label;
      button.addEventListener('click', () => applyReadingGuideModePreset(entry.mode));
      modeGrid.appendChild(button);
      modeButtons.push({ mode: entry.mode, button });
    });
    modeGroup.appendChild(modeGrid);
    ruleSection.appendChild(modeGroup);
    ...
    const opacitySliderId = `${baseId}-opacity`;
    opacityLabel.setAttribute('for', opacitySliderId);
    ...
    opacitySlider.id = opacitySliderId;
    ...
    const heightSliderId = `${baseId}-height`;
    heightLabel.setAttribute('for', heightSliderId);
    ...
    heightSlider.id = heightSliderId;
  ...
  function updateDyslexiaInstanceUI(instance){
    ...
    if(Array.isArray(presetButtonEntries)){
      presetButtonEntries.forEach(entry => {
        if(!entry || !entry.button){ return; }
        entry.button.disabled = !active;
        entry.button.setAttribute('aria-disabled', active ? 'false' : 'true');
      });
    }
    if(Array.isArray(patternButtonEntries)){
      const currentPattern = normalizeDyslexiaPatternPreset(dyslexiaSettings.patternPreset);
      patternButtonEntries.forEach(entry => {
        if(!entry || !entry.button){ return; }
        entry.button.disabled = !active;
        entry.button.setAttribute('aria-disabled', active ? 'false' : 'true');
        entry.button.setAttribute('aria-pressed', entry.key === currentPattern ? 'true' : 'false');
        entry.button.classList.toggle('is-active', entry.key === currentPattern);
      });
    }
    if(colorInput){
      colorInput.disabled = !active;
    ...
    if(letterSpacingSlider){
      letterSpacingSlider.disabled = !active;
      setInputValue(letterSpacingSlider, String(clampDyslexiaLetterSpacing(dyslexiaSettings.letterSpacing)));
    }
    if(letterSpacingValue){
      letterSpacingValue.textContent = formatDyslexiaLetterSpacing(dyslexiaSettings.letterSpacing);
    }
    if(wordSpacingSlider){
      wordSpacingSlider.disabled = !active;
      setInputValue(wordSpacingSlider, String(clampDyslexiaWordSpacing(dyslexiaSettings.wordSpacing)));
    }
    if(wordSpacingValue){
      wordSpacingValue.textContent = formatDyslexiaWordSpacing(dyslexiaSettings.wordSpacing);
    }
    if(paragraphSpacingSlider){
      paragraphSpacingSlider.disabled = !active;
      setInputValue(paragraphSpacingSlider, String(clampDyslexiaParagraphSpacing(dyslexiaSettings.paragraphSpacing)));
    }
    if(paragraphSpacingValue){
      paragraphSpacingValue.textContent = formatDyslexiaParagraphSpacing(dyslexiaSettings.paragraphSpacing);
    }
    if(lineLengthSlider){
      lineLengthSlider.disabled = !active;
      setInputValue(lineLengthSlider, String(clampDyslexiaLineLength(dyslexiaSettings.lineLength)));
    }
    if(lineLengthValue){
      lineLengthValue.textContent = formatDyslexiaLineLength(dyslexiaSettings.lineLength);
    }
    if(alignInput){
      alignInput.disabled = !active;
      setCheckboxState(alignInput, !!dyslexiaSettings.alignLeft);
    }
    if(removePatternsInput){
      removePatternsInput.disabled = !active;
      setCheckboxState(removePatternsInput, !!dyslexiaSettings.removePatterns);
    }
    if(italicInput){
    ...
    if(message){
      ...
      const shouldShow = active && !dyslexiaSettings.letter && !getDyslexiaActivePatternList().length && !!warning;
  ...
  function getDyslexiaActivePatternList(){
    const presetKey = normalizeDyslexiaPatternPreset(dyslexiaSettings.patternPreset);
    const presetPatterns = DYSLEXIA_PATTERN_PRESETS[presetKey] || [];
    return presetPatterns
      .filter(pattern => typeof pattern === 'string' && pattern)
      .map(pattern => Array.from(pattern))
      .filter(patternChars => patternChars.length)
      .sort((a, b) => b.length - a.length);
  }

  function normalizeDyslexiaPatternText(value, accentInclusive){
    const lower = typeof value === 'string' ? value.toLocaleLowerCase() : '';
    return accentInclusive ? stripDyslexiaAccents(lower).toLocaleLowerCase() : lower;
  }

  function findDyslexiaPatternMatch(chars, index, patterns, accentInclusive){
    if(!Array.isArray(chars) || !Array.isArray(patterns) || !patterns.length){ return 0; }
    for(let i = 0; i < patterns.length; i++){
      const patternChars = patterns[i];
      if(!Array.isArray(patternChars) || !patternChars.length){ continue; }
      if(index + patternChars.length > chars.length){ continue; }
      const source = chars.slice(index, index + patternChars.length).join('');
      const sourceCompare = normalizeDyslexiaPatternText(source, accentInclusive);
      const patternCompare = normalizeDyslexiaPatternText(patternChars.join(''), accentInclusive);
      if(sourceCompare && sourceCompare === patternCompare){
  ...
  function buildDyslexiaFragments(text, baseLetters, normalizedLetters, accentInclusive, color, activePatterns){
    if(!text){ return null; }
    ...
    for(let i = 0; i < chars.length; i++){
      const patternLength = findDyslexiaPatternMatch(chars, i, activePatterns, accentInclusive);
      if(patternLength > 0){
        matched = true;
        if(buffer){
        ...
        span.textContent = chars.slice(i, i + patternLength).join('');
  ...
  function applyDyslexiaHighlights(){
    ...
    const activePatterns = getDyslexiaActivePatternList();
    if(!letter && !activePatterns.length){ return; }
    ...
    while(walker.nextNode()){
      ...
      const fragments = buildDyslexiaFragments(node.nodeValue, baseLetters, normalizedLetters, accentInclusive, color, activePatterns);
  ...
  function setDyslexiaLetter(value){
    ...
    clearDyslexiaHighlights();
    if(dyslexiaActive && (sanitized || getDyslexiaActivePatternList().length)){
  ...
  function setDyslexiaAccentInclusive(value){
    ...
    if(changed){ persistDyslexiaSettings(); }
    if(dyslexiaActive && (dyslexiaSettings.letter || getDyslexiaActivePatternList().length)){
      applyDyslexiaHighlights();
    }
    syncDyslexiaInstances();
  }
  ...
  function setDyslexiaLetterSpacing(value, options = {}){
    const next = clampDyslexiaLetterSpacing(value);
    const changed = clampDyslexiaLetterSpacing(dyslexiaSettings.letterSpacing) !== next;
    dyslexiaSettings.letterSpacing = next;
    if(changed || options.force){
      updateDyslexiaStyles();
      syncDyslexiaInstances();
      if(options.persist !== false){ persistDyslexiaSettings(); }
  ...
  function setDyslexiaWordSpacing(value, options = {}){
    const next = clampDyslexiaWordSpacing(value);
    const changed = clampDyslexiaWordSpacing(dyslexiaSettings.wordSpacing) !== next;
    dyslexiaSettings.wordSpacing = next;
    if(changed || options.force){
      updateDyslexiaStyles();
      syncDyslexiaInstances();
      if(options.persist !== false){ persistDyslexiaSettings(); }
  ...
  function setDyslexiaParagraphSpacing(value, options = {}){
    const next = clampDyslexiaParagraphSpacing(value);
    const changed = clampDyslexiaParagraphSpacing(dyslexiaSettings.paragraphSpacing) !== next;
    dyslexiaSettings.paragraphSpacing = next;
    if(changed || options.force){
      updateDyslexiaStyles();
      syncDyslexiaInstances();
      if(options.persist !== false){ persistDyslexiaSettings(); }
  ...
  function setDyslexiaLineLength(value, options = {}){
    const next = clampDyslexiaLineLength(value);
    const changed = clampDyslexiaLineLength(dyslexiaSettings.lineLength) !== next;
    dyslexiaSettings.lineLength = next;
    if(changed || options.force){
      updateDyslexiaStyles();
      syncDyslexiaInstances();
      if(options.persist !== false){ persistDyslexiaSettings(); }
  ...
  function setDyslexiaAlignLeft(value, options = {}){
    const next = !!value;
    const changed = !!dyslexiaSettings.alignLeft !== next;
    dyslexiaSettings.alignLeft = next;
    if(changed || options.force){
      updateDyslexiaStyles();
      syncDyslexiaInstances();
      if(options.persist !== false){ persistDyslexiaSettings(); }
  ...
  function setDyslexiaRemovePatterns(value, options = {}){
    const next = !!value;
    const changed = !!dyslexiaSettings.removePatterns !== next;
    dyslexiaSettings.removePatterns = next;
    if(changed || options.force){
      updateDyslexiaStyles();
      syncDyslexiaInstances();
      if(options.persist !== false){ persistDyslexiaSettings(); }
  ...
  function setDyslexiaPatternPreset(value, options = {}){
    const next = normalizeDyslexiaPatternPreset(value);
    const changed = normalizeDyslexiaPatternPreset(dyslexiaSettings.patternPreset) !== next;
    dyslexiaSettings.patternPreset = next;
    if(changed || options.force){
      clearDyslexiaHighlights();
      if(dyslexiaActive){
        applyDyslexiaHighlights();
  ...
  function applyDyslexiaLayoutPreset(key, options = {}){
    const preset = DYSLEXIA_LAYOUT_PRESETS[key];
    if(!preset){ return; }
    dyslexiaSettings = Object.assign({}, dyslexiaSettings, {
      font: normalizeDyslexiaFont(preset.font),
      fontSize: clampDyslexiaFontSize(preset.fontSize),
      lineHeight: clampDyslexiaLineHeight(preset.lineHeight),
      letterSpacing: clampDyslexiaLetterSpacing(preset.letterSpacing),
      wordSpacing: clampDyslexiaWordSpacing(preset.wordSpacing),
      paragraphSpacing: clampDyslexiaParagraphSpacing(preset.paragraphSpacing),
      lineLength: clampDyslexiaLineLength(preset.lineLength),
      alignLeft: !!preset.alignLeft,
      removePatterns: !!preset.removePatterns,
      disableItalic: !!preset.disableItalic,
      disableBold: !!preset.disableBold,
    });
    updateDyslexiaStyles();
    syncDyslexiaInstances();
    if(options.persist !== false){ persistDyslexiaSettings(); }
  ...
  function resetDyslexiaTypography(){
    ...
    dyslexiaSettings.letterSpacing = DYSLEXIA_DEFAULTS.letterSpacing;
    dyslexiaSettings.wordSpacing = DYSLEXIA_DEFAULTS.wordSpacing;
    dyslexiaSettings.paragraphSpacing = DYSLEXIA_DEFAULTS.paragraphSpacing;
    dyslexiaSettings.lineLength = DYSLEXIA_DEFAULTS.lineLength;
    dyslexiaSettings.alignLeft = DYSLEXIA_DEFAULTS.alignLeft;
    dyslexiaSettings.removePatterns = DYSLEXIA_DEFAULTS.removePatterns;
  ...
  function handleDyslexiaLetterSpacingInput(value){
    setDyslexiaLetterSpacing(value, { persist: false });
  }

  function handleDyslexiaLetterSpacingChange(value){
    setDyslexiaLetterSpacing(value, { force: true });
  }

  function handleDyslexiaWordSpacingInput(value){
    setDyslexiaWordSpacing(value, { persist: false });
  }

  function handleDyslexiaWordSpacingChange(value){
    setDyslexiaWordSpacing(value, { force: true });
  }

  function handleDyslexiaParagraphSpacingInput(value){
    setDyslexiaParagraphSpacing(value, { persist: false });
  }

  function handleDyslexiaParagraphSpacingChange(value){
    setDyslexiaParagraphSpacing(value, { force: true });
  }

  function handleDyslexiaLineLengthInput(value){
    setDyslexiaLineLength(value, { persist: false });
  }

  function handleDyslexiaLineLengthChange(value){
    setDyslexiaLineLength(value, { force: true });
  }
  ...
  function createDyslexiaCard(feature){
    ...
    const texts = {
      ...
      preset_label: typeof settings.preset_label === 'string' ? settings.preset_label : '',
      preset_soft_label: typeof settings.preset_soft_label === 'string' ? settings.preset_soft_label : '',
      preset_strong_label: typeof settings.preset_strong_label === 'string' ? settings.preset_strong_label : '',
      preset_calm_label: typeof settings.preset_calm_label === 'string' ? settings.preset_calm_label : '',
      pattern_label: typeof settings.pattern_label === 'string' ? settings.pattern_label : '',
      pattern_none_label: typeof settings.pattern_none_label === 'string' ? settings.pattern_none_label : '',
      pattern_graphemes_label: typeof settings.pattern_graphemes_label === 'string' ? settings.pattern_graphemes_label : '',
      pattern_confusions_label: typeof settings.pattern_confusions_label === 'string' ? settings.pattern_confusions_label : '',
      pattern_morphemes_label: typeof settings.pattern_morphemes_label === 'string' ? settings.pattern_morphemes_label : '',
      ...
      spacing_label: typeof settings.spacing_label === 'string' ? settings.spacing_label : '',
      letter_spacing_label: typeof settings.letter_spacing_label === 'string' ? settings.letter_spacing_label : '',
      word_spacing_label: typeof settings.word_spacing_label === 'string' ? settings.word_spacing_label : '',
      paragraph_spacing_label: typeof settings.paragraph_spacing_label === 'string' ? settings.paragraph_spacing_label : '',
      line_length_label: typeof settings.line_length_label === 'string' ? settings.line_length_label : '',
      align_left_label: typeof settings.align_left_label === 'string' ? settings.align_left_label : '',
      remove_patterns_label: typeof settings.remove_patterns_label === 'string' ? settings.remove_patterns_label : '',
    ...
    const presetField = document.createElement('fieldset');
    presetField.className = 'a11y-dyslexie__fieldset a11y-dyslexie__fieldset--presets';
    const presetLegend = document.createElement('legend');
    presetLegend.className = 'a11y-dyslexie__legend';
    presetLegend.textContent = texts.preset_label || 'Réglages rapides';
    presetField.appendChild(presetLegend);
    const presetGrid = document.createElement('div');
    presetGrid.className = 'a11y-dyslexie__button-grid';
    const presetButtonEntries = [];
    [
      { key: 'soft', label: texts.preset_soft_label || 'Aéré doux' },
      { key: 'strong', label: texts.preset_strong_label || 'Aéré fort' },
      { key: 'calm', label: texts.preset_calm_label || 'Sans motifs' },
    ].forEach(preset => {
      const button = document.createElement('button');
      button.type = 'button';
      button.className = 'a11y-dyslexie__choice';
      button.dataset.preset = preset.key;
      button.textContent = preset.label;
      button.addEventListener('click', () => {
        if(!dyslexiaActive){ toggleFeature(feature.slug, true); }
        applyDyslexiaLayoutPreset(preset.key);
      });
      presetGrid.appendChild(button);
      presetButtonEntries.push({ key: preset.key, button });
    });
    presetField.appendChild(presetGrid);
    controls.appendChild(presetField);

    const patternField = document.createElement('fieldset');
    patternField.className = 'a11y-dyslexie__fieldset a11y-dyslexie__fieldset--patterns';
    const patternLegend = document.createElement('legend');
    patternLegend.className = 'a11y-dyslexie__legend';
    patternLegend.textContent = texts.pattern_label || 'Patterns de surlignage';
    patternField.appendChild(patternLegend);
    const patternGrid = document.createElement('div');
    patternGrid.className = 'a11y-dyslexie__button-grid';
    const patternButtonEntries = [];
    [
      { key: 'none', label: texts.pattern_none_label || 'Aucun' },
      { key: 'graphemes', label: texts.pattern_graphemes_label || 'Graphèmes' },
      { key: 'confusions', label: texts.pattern_confusions_label || 'Confusions' },
      { key: 'morphemes', label: texts.pattern_morphemes_label || 'Morphèmes' },
    ].forEach(pattern => {
      const button = document.createElement('button');
      button.type = 'button';
      button.className = 'a11y-dyslexie__choice';
      button.dataset.patternPreset = pattern.key;
      button.setAttribute('aria-pressed', normalizeDyslexiaPatternPreset(dyslexiaSettings.patternPreset) === pattern.key ? 'true' : 'false');
      button.textContent = pattern.label;
      button.addEventListener('click', () => {
        if(pattern.key !== 'none' && !dyslexiaActive){ toggleFeature(feature.slug, true); }
        setDyslexiaPatternPreset(pattern.key, { force: true });
      });
      patternGrid.appendChild(button);
      patternButtonEntries.push({ key: pattern.key, button });
    });
    patternField.appendChild(patternGrid);
    controls.appendChild(patternField);
    ...
    const spacingField = document.createElement('fieldset');
    spacingField.className = 'a11y-dyslexie__fieldset a11y-dyslexie__fieldset--spacing';
    const spacingLegend = document.createElement('legend');
    spacingLegend.className = 'a11y-dyslexie__legend';
    spacingLegend.textContent = texts.spacing_label || 'Espacement du texte';
    spacingField.appendChild(spacingLegend);

    const letterSpacingField = document.createElement('div');
    letterSpacingField.className = 'a11y-dyslexie__field a11y-dyslexie__field--range';
    const letterSpacingId = `${baseId}-letter-spacing`;
    const letterSpacingLabel = document.createElement('label');
    letterSpacingLabel.setAttribute('for', letterSpacingId);
    letterSpacingLabel.textContent = texts.letter_spacing_label || 'Espacement des lettres';
    const letterSpacingValue = document.createElement('span');
    letterSpacingValue.className = 'a11y-dyslexie__value';
    letterSpacingValue.textContent = formatDyslexiaLetterSpacing(dyslexiaSettings.letterSpacing);
    ...
    letterSpacingSlider.id = letterSpacingId;
    letterSpacingSlider.className = 'a11y-dyslexie__range';
    letterSpacingSlider.min = String(DYSLEXIA_LETTER_SPACING_MIN);
    letterSpacingSlider.max = String(DYSLEXIA_LETTER_SPACING_MAX);
    letterSpacingSlider.step = String(DYSLEXIA_LETTER_SPACING_STEP);
    letterSpacingSlider.value = String(clampDyslexiaLetterSpacing(dyslexiaSettings.letterSpacing));
    letterSpacingField.appendChild(letterSpacingLabel);
    letterSpacingField.appendChild(letterSpacingSlider);
    spacingField.appendChild(letterSpacingField);

    const wordSpacingField = document.createElement('div');
    wordSpacingField.className = 'a11y-dyslexie__field a11y-dyslexie__field--range';
    const wordSpacingId = `${baseId}-word-spacing`;
    const wordSpacingLabel = document.createElement('label');
    wordSpacingLabel.setAttribute('for', wordSpacingId);
    wordSpacingLabel.textContent = texts.word_spacing_label || 'Espacement des mots';
    const wordSpacingValue = document.createElement('span');
    wordSpacingValue.className = 'a11y-dyslexie__value';
    wordSpacingValue.textContent = formatDyslexiaWordSpacing(dyslexiaSettings.wordSpacing);
    wordSpacingLabel.appendChild(wordSpacingValue);
    const wordSpacingSlider = document.createElement('input');
    wordSpacingSlider.type = 'range';
    wordSpacingSlider.id = wordSpacingId;
    wordSpacingSlider.className = 'a11y-dyslexie__range';
    wordSpacingSlider.min = String(DYSLEXIA_WORD_SPACING_MIN);
    wordSpacingSlider.max = String(DYSLEXIA_WORD_SPACING_MAX);
    wordSpacingSlider.step = String(DYSLEXIA_WORD_SPACING_STEP);
    wordSpacingSlider.value = String(clampDyslexiaWordSpacing(dyslexiaSettings.wordSpacing));
    wordSpacingField.appendChild(wordSpacingLabel);
    wordSpacingField.appendChild(wordSpacingSlider);
    spacingField.appendChild(wordSpacingField);

    const paragraphSpacingField = document.createElement('div');
    paragraphSpacingField.className = 'a11y-dyslexie__field a11y-dyslexie__field--range';
    const paragraphSpacingId = `${baseId}-paragraph-spacing`;
    const paragraphSpacingLabel = document.createElement('label');
    paragraphSpacingLabel.setAttribute('for', paragraphSpacingId);
    paragraphSpacingLabel.textContent = texts.paragraph_spacing_label || 'Espacement des paragraphes';
    const paragraphSpacingValue = document.createElement('span');
    paragraphSpacingValue.className = 'a11y-dyslexie__value';
    paragraphSpacingValue.textContent = formatDyslexiaParagraphSpacing(dyslexiaSettings.paragraphSpacing);
    paragraphSpacingLabel.appendChild(paragraphSpacingValue);
    const paragraphSpacingSlider = document.createElement('input');
    paragraphSpacingSlider.type = 'range';
    paragraphSpacingSlider.id = paragraphSpacingId;
    paragraphSpacingSlider.className = 'a11y-dyslexie__range';
    paragraphSpacingSlider.min = String(DYSLEXIA_PARAGRAPH_SPACING_MIN);
    paragraphSpacingSlider.max = String(DYSLEXIA_PARAGRAPH_SPACING_MAX);
    paragraphSpacingSlider.step = String(DYSLEXIA_PARAGRAPH_SPACING_STEP);
    paragraphSpacingSlider.value = String(clampDyslexiaParagraphSpacing(dyslexiaSettings.paragraphSpacing));
    paragraphSpacingField.appendChild(paragraphSpacingLabel);
    paragraphSpacingField.appendChild(paragraphSpacingSlider);
    spacingField.appendChild(paragraphSpacingField);

    const lineLengthField = document.createElement('div');
    lineLengthField.className = 'a11y-dyslexie__field a11y-dyslexie__field--range';
    const lineLengthId = `${baseId}-line-length`;
    const lineLengthLabel = document.createElement('label');
    lineLengthLabel.setAttribute('for', lineLengthId);
    lineLengthLabel.textContent = texts.line_length_label || 'Longueur de ligne';
    const lineLengthValue = document.createElement('span');
    lineLengthValue.className = 'a11y-dyslexie__value';
    lineLengthValue.textContent = formatDyslexiaLineLength(dyslexiaSettings.lineLength);
    lineLengthLabel.appendChild(lineLengthValue);
    const lineLengthSlider = document.createElement('input');
    lineLengthSlider.type = 'range';
    lineLengthSlider.id = lineLengthId;
    lineLengthSlider.className = 'a11y-dyslexie__range';
    lineLengthSlider.min = String(DYSLEXIA_LINE_LENGTH_MIN);
    lineLengthSlider.max = String(DYSLEXIA_LINE_LENGTH_MAX);
    lineLengthSlider.step = String(DYSLEXIA_LINE_LENGTH_STEP);
    lineLengthSlider.value = String(clampDyslexiaLineLength(dyslexiaSettings.lineLength));
    lineLengthField.appendChild(lineLengthLabel);
    lineLengthField.appendChild(lineLengthSlider);
    spacingField.appendChild(lineLengthField);

    const layoutOptions = document.createElement('div');
    layoutOptions.className = 'a11y-dyslexie__style-options';
    const alignLabel = document.createElement('label');
    alignLabel.className = 'a11y-dyslexie__checkbox';
    const alignInput = document.createElement('input');
    alignInput.type = 'checkbox';
    alignInput.id = `${baseId}-align-left`;
    alignInput.className = 'a11y-dyslexie__checkbox-input';
    const alignText = document.createElement('span');
    alignText.textContent = texts.align_left_label || 'Aligner le texte à gauche';
    alignLabel.appendChild(alignInput);
    alignLabel.appendChild(alignText);
    layoutOptions.appendChild(alignLabel);
    const removePatternsLabel = document.createElement('label');
    removePatternsLabel.className = 'a11y-dyslexie__checkbox';
    const removePatternsInput = document.createElement('input');
    removePatternsInput.type = 'checkbox';
    removePatternsInput.id = `${baseId}-remove-patterns`;
    removePatternsInput.className = 'a11y-dyslexie__checkbox-input';
    const removePatternsText = document.createElement('span');
    removePatternsText.textContent = texts.remove_patterns_label || 'Supprimer les motifs visuels';
    removePatternsLabel.appendChild(removePatternsInput);
    removePatternsLabel.appendChild(removePatternsText);
    layoutOptions.appendChild(removePatternsLabel);
    spacingField.appendChild(layoutOptions);
    controls.appendChild(spacingField);
    ...
    letterSpacingSlider.addEventListener('input', () => handleDyslexiaLetterSpacingInput(letterSpacingSlider.value));
    letterSpacingSlider.addEventListener('change', () => handleDyslexiaLetterSpacingChange(letterSpacingSlider.value));
    wordSpacingSlider.addEventListener('input', () => handleDyslexiaWordSpacingInput(wordSpacingSlider.value));
    wordSpacingSlider.addEventListener('change', () => handleDyslexiaWordSpacingChange(wordSpacingSlider.value));
    paragraphSpacingSlider.addEventListener('input', () => handleDyslexiaParagraphSpacingInput(paragraphSpacingSlider.value));
    paragraphSpacingSlider.addEventListener('change', () => handleDyslexiaParagraphSpacingChange(paragraphSpacingSlider.value));
    lineLengthSlider.addEventListener('input', () => handleDyslexiaLineLengthInput(lineLengthSlider.value));
    lineLengthSlider.addEventListener('change', () => handleDyslexiaLineLengthChange(lineLengthSlider.value));
    alignInput.addEventListener('change', () => setDyslexiaAlignLeft(alignInput.checked, { force: true }));
    removePatternsInput.addEventListener('change', () => setDyslexiaRemovePatterns(removePatternsInput.checked, { force: true }));
  ...
  const DEFAULT_PROFILE_PRESET_DEFINITIONS = [
    {
      key: 'reading',
      labelKey: 'profile_reading_label',
      hintKey: 'profile_reading_hint',
      fallbackLabel: 'Lecture confortable',
      fallbackHint: 'Guide de lecture, réglages de texte et repères de structure.',
      features: ['cognitif-reading-guide', 'cognitif-dyslexie', 'audition-text-to-speech']
    },
    {
      key: 'visual',
      labelKey: 'profile_visual_label',
      hintKey: 'profile_visual_hint',
      fallbackLabel: 'Contraste renforcé',
      fallbackHint: 'Luminosité, contraste et atténuation visuelle comme point de départ.',
      features: ['luminosite-reglages', 'vision-migraine']
    },
    {
      key: 'focus',
      labelKey: 'profile_focus_label',
      hintKey: 'profile_focus_hint',
      fallbackLabel: 'Concentration',
      fallbackHint: 'Réduction des distractions et aide au suivi de lecture.',
      features: ['cognitif-reading-guide', 'cognitif-reduire-distractions']
    },
    {
      key: 'text',
      labelKey: 'profile_text_label',
      hintKey: 'profile_text_hint',
      fallbackLabel: 'Texte seul',
      fallbackHint: 'Page recentrée sur le texte, avec images et médias masqués.',
      features: ['lecture-texte-seul', 'lecture-masquer-images', 'cognitif-reduire-distractions']
    },
    {
      key: 'motor',
      labelKey: 'profile_motor_label',
      hintKey: 'profile_motor_hint',
      fallbackLabel: 'Navigation facilitée',
      fallbackHint: 'Boutons plus visibles et curseur renforcé.',
      features: ['moteur-boutons', 'moteur-curseur']
    },
    {
      key: 'safety',
      labelKey: 'profile_safety_label',
      hintKey: 'profile_safety_hint',
      fallbackLabel: 'Sécurité visuelle',
      fallbackHint: 'Réduction des animations, vidéos automatiques et déclencheurs visuels potentiels.',
      features: ['epilepsie-protection']
    }
  ];

  function getProfileFeatureSettings(){
    for(const section of sectionsData){
      const features = section && Array.isArray(section.features) ? section.features : [];
      for(const feature of features){
        if(feature && feature.template === 'profile-presets'){
          return feature.settings && typeof feature.settings === 'object' ? feature.settings : {};
        }
      }
    }
    return {};
  ...
  function normalizeProfileFeatureList(features){
    const rawFeatures = Array.isArray(features) ? features : [];
    const seen = new Set();
    const normalized = [];
    rawFeatures.forEach(feature => {
      const slug = typeof feature === 'string' ? feature.trim() : '';
  ...
  function cloneDefaultProfileDefinition(profile){
    return Object.assign({}, profile, {
      features: Array.isArray(profile.features) ? profile.features.slice() : []
    });
  }

  function buildConfiguredProfilePresetDefinitions(){
    const settings = getProfileFeatureSettings();
    const configuredProfiles = Array.isArray(settings.profiles) ? settings.profiles : [];
    const defaultsByKey = new Map(DEFAULT_PROFILE_PRESET_DEFINITIONS.map(profile => [profile.key, profile]));
    const definitions = configuredProfiles
      .map(profile => {
        if(!profile || typeof profile !== 'object'){ return null; }
        const key = typeof profile.key === 'string' ? profile.key.trim() : '';
        const defaultProfile = defaultsByKey.get(key);
        if(!defaultProfile){ return null; }

        const definition = cloneDefaultProfileDefinition(defaultProfile);
        const label = typeof profile.label === 'string' ? profile.label.trim() : '';
        const hint = typeof profile.hint === 'string' ? profile.hint.trim() : '';
        const features = normalizeProfileFeatureList(profile.features);

        if(label){ definition.label = label; }
        if(hint){ definition.hint = hint; }
        if(features.length){ definition.features = features; }

        return definition.features.length ? definition : null;
      })
      .filter(Boolean);

    return definitions.length
      ? definitions
      : DEFAULT_PROFILE_PRESET_DEFINITIONS.map(cloneDefaultProfileDefinition);
  }

  const PROFILE_PRESET_DEFINITIONS = buildConfiguredProfilePresetDefinitions();
  const PROFILE_MANAGED_FEATURES = Array.from(new Set(PROFILE_PRESET_DEFINITIONS.flatMap(profile => profile.features)));
  const profilePresetInstances = new Set();

  function getProfilePresetDefinition(profileKey){
    return PROFILE_PRESET_DEFINITIONS.find(profile => profile.key === profileKey) || null;
  }

  function normalizeProfilePresetKey(profileKey){
    return getProfilePresetDefinition(profileKey) ? profileKey : '';
  }

  function getActiveProfileFeatureCount(profile){
    if(!profile || !Array.isArray(profile.features)){ return 0; }
    return profile.features.reduce((count, slug) => count + (featureState[slug] ? 1 : 0), 0);
  }

  function isProfileFullyActive(profile){
    return !!(profile && Array.isArray(profile.features) && profile.features.length && getActiveProfileFeatureCount(profile) === profile.features.length);
  }

  function getActiveProfileKey(preferredKey = ''){
    const preferredProfile = getProfilePresetDefinition(preferredKey);
    if(isProfileFullyActive(preferredProfile)){ return preferredProfile.key; }

    const activeProfiles = PROFILE_PRESET_DEFINITIONS
      .filter(profile => isProfileFullyActive(profile))
      .sort((a, b) => b.features.length - a.features.length);

    return activeProfiles.length ? activeProfiles[0].key : '';
  }

  function formatProfileFeatureCount(profile, activeCount){
    const total = profile && Array.isArray(profile.features) ? profile.features.length : 0;
    if(activeCount > 0){
      return `${activeCount}/${total} actif${activeCount > 1 ? 's' : ''}`;
    }
    return `${total} réglage${total > 1 ? 's' : ''}`;
  }

  function syncProfilePresetInstances(){
    profilePresetInstances.forEach(instance => {
      if(!instance || !instance.article || !instance.article.isConnected){
        profilePresetInstances.delete(instance);
        return;
      }
      try {
        instance.sync();
  ...
  function getFeatureSetting(feature, key, fallback=''){
    const settings = feature && feature.settings && typeof feature.settings === 'object' ? feature.settings : {};
    const value = settings[key];
    return typeof value === 'string' && value ? value : fallback;
  }

  function getFeatureSettingList(feature, key){
    const settings = feature && feature.settings && typeof feature.settings === 'object' ? feature.settings : {};
    const value = settings[key];
  ...
  function loadLastProfileKey(){
    try {
      const value = localStorage.getItem(PROFILE_PRESET_KEY);
      return typeof value === 'string' ? normalizeProfilePresetKey(value) : '';
    } catch(err){
      return '';
    }
  }

  function persistLastProfileKey(key){
    const normalizedKey = normalizeProfilePresetKey(key);
    try {
      if(normalizedKey){ localStorage.setItem(PROFILE_PRESET_KEY, normalizedKey); }
      else { localStorage.removeItem(PROFILE_PRESET_KEY); }
    } catch(err){ /* ignore */ }
  }

  let currentProfilePresetKey = loadLastProfileKey();

  function applyProfileSettingPreset(profileKey){
    switch(profileKey){
      case 'reading':
        applyDyslexiaLayoutPreset('soft');
        setDyslexiaPatternPreset('graphemes', { force: true });
        applyReadingGuideModePreset('bar');
        setReadingGuideSetting('summaryEnabled', true);
        setReadingGuideSetting('syllableEnabled', true);
        setReadingGuideSetting('focusEnabled', false);
        updateTtsSettings({ mode: 'selection', rate: 0.75 });
        break;
      case 'focus':
        applyReadingGuideModePreset('lightbox');
        setReadingGuideSetting('focusEnabled', true);
        setReadingGuideSetting('summaryEnabled', false);
        setReadingGuideSetting('syllableEnabled', false);
        break;
      default:
        break;
    }
  ...
  function createProfilePresetsCard(feature){
    const article = document.createElement('article');
    article.className = 'a11y-card a11y-card--profiles';
    article.setAttribute('data-role', 'feature-card');
    ...
    const labelEl = document.createElement('span');
    labelEl.className = 'label';
    labelEl.textContent = feature.label || 'Profils';
    meta.appendChild(labelEl);
    ...
    const intro = getFeatureSetting(feature, 'intro', '');
    if(intro){
      const introEl = document.createElement('p');
      introEl.className = 'a11y-profile__intro';
      introEl.textContent = intro;
      article.appendChild(introEl);
    }

    const list = document.createElement('div');
    list.className = 'a11y-profile-list';
    article.appendChild(list);

    const status = document.createElement('div');
    status.className = 'a11y-profile__status';
    status.setAttribute('role', 'status');
    status.setAttribute('aria-live', 'polite');
    const liveLabel = getFeatureSetting(feature, 'live_region_label', '');
    if(liveLabel){ status.setAttribute('aria-label', liveLabel); }

    const last = document.createElement('p');
    last.className = 'a11y-profile__last';

    const applyLabel = getFeatureSetting(feature, 'apply_label', 'Appliquer');
    const activeProfileLabel = getFeatureSetting(feature, 'active_profile_label', 'Profil appliqué');
    const partialProfileLabel = getFeatureSetting(feature, 'partial_profile_label', 'Profil partiellement actif');
    const activeBadgeLabel = getFeatureSetting(feature, 'active_badge_label', 'Actif');
    const partialBadgeLabel = getFeatureSetting(feature, 'partial_badge_label', 'Partiel');
    const lastProfileLabel = getFeatureSetting(feature, 'last_profile_label', 'Dernier profil appliqué');
    const clearLabel = getFeatureSetting(feature, 'clear_label', 'Désactiver ce profil');
    const clearedStatus = getFeatureSetting(feature, 'cleared_status', 'Profil désactivé.');
    let clearButton = null;
    const profileButtonEntries = [];

    const getProfileLabel = profile => profile.label || getFeatureSetting(feature, profile.labelKey, profile.fallbackLabel);
    const getProfileHint = profile => profile.hint || getFeatureSetting(feature, profile.hintKey, profile.fallbackHint);

    const getClearProfile = () => {
      const activeProfileKey = getActiveProfileKey(currentProfilePresetKey);
      return getProfilePresetDefinition(currentProfilePresetKey) || getProfilePresetDefinition(activeProfileKey);
    };

    const syncProfileButtons = () => {
      const activeProfileKey = getActiveProfileKey(currentProfilePresetKey);
      profileButtonEntries.forEach(entry => {
        const activeCount = getActiveProfileFeatureCount(entry.profile);
        const isActive = entry.profile.key === activeProfileKey;
        const isPartial = !isActive && activeCount > 0;
        entry.button.classList.toggle('is-active', isActive);
        entry.button.classList.toggle('is-partial', isPartial);
        entry.button.setAttribute('aria-pressed', isActive ? 'true' : 'false');
        entry.button.setAttribute('aria-label', `${isActive ? activeProfileLabel : (isPartial ? partialProfileLabel : applyLabel)} : ${entry.label}`);
        entry.state.textContent = isActive ? activeBadgeLabel : (isPartial ? partialBadgeLabel : '');
        entry.state.hidden = !isActive && !isPartial;
        if(entry.count){
          entry.count.textContent = formatProfileFeatureCount(entry.profile, activeCount);
        }
      });
      if(clearButton){
        const clearProfile = getClearProfile();
        const clearProfileLabel = clearProfile ? getProfileLabel(clearProfile) : '';
        clearButton.disabled = !clearProfile;
        clearButton.setAttribute('aria-disabled', clearProfile ? 'false' : 'true');
        clearButton.textContent = clearLabel;
        clearButton.setAttribute('aria-label', clearProfileLabel ? `${clearLabel} : ${clearProfileLabel}` : clearLabel);
      }
    };

    const updateLastProfile = () => {
      currentProfilePresetKey = normalizeProfilePresetKey(currentProfilePresetKey);
      const profile = getProfilePresetDefinition(currentProfilePresetKey);
      if(profile){
        const label = getProfileLabel(profile);
        last.textContent = `${lastProfileLabel} : ${label}`;
        last.hidden = false;
      } else {
        last.textContent = '';
        last.hidden = true;
    ...
    PROFILE_PRESET_DEFINITIONS.forEach(profile => {
      const profileButton = document.createElement('button');
      profileButton.type = 'button';
      profileButton.className = 'a11y-profile-option';
      profileButton.dataset.profileKey = profile.key;
      const label = getProfileLabel(profile);
      const hint = getProfileHint(profile);
      profileButton.setAttribute('aria-label', `${applyLabel} : ${label}`);
      profileButton.setAttribute('aria-pressed', 'false');

      const title = document.createElement('span');
      title.className = 'a11y-profile-option__title';
      title.textContent = label;
      profileButton.appendChild(title);

      const hintEl = document.createElement('span');
      hintEl.className = 'a11y-profile-option__hint';
      hintEl.textContent = hint;
      profileButton.appendChild(hintEl);

      const count = document.createElement('span');
      count.className = 'a11y-profile-option__count';
      count.textContent = `${profile.features.length} réglage${profile.features.length > 1 ? 's' : ''}`;
      profileButton.appendChild(count);

      const state = document.createElement('span');
      state.className = 'a11y-profile-option__state';
      state.textContent = activeBadgeLabel;
      state.hidden = true;
      profileButton.appendChild(state);

      profileButton.addEventListener('click', () => {
        const previousProfile = getProfilePresetDefinition(currentProfilePresetKey);
        const nextFeatureKeys = new Set(profile.features);
        if(previousProfile && previousProfile.key !== profile.key){
          previousProfile.features.forEach(slug => {
            if(!nextFeatureKeys.has(slug)){
              toggleFeature(slug, false);
            }
          });
        }
        profile.features.forEach(slug => toggleFeature(slug, true));
        applyProfileSettingPreset(profile.key);
        currentProfilePresetKey = profile.key;
        persistLastProfileKey(profile.key);
        syncProfilePresetInstances();
        const appliedStatus = getFeatureSetting(feature, 'applied_status', 'Profil appliqué.');
        status.textContent = `${appliedStatus} ${label}`;
      });

      profileButtonEntries.push({ profile, button: profileButton, label, count, state });
      list.appendChild(profileButton);
    });

    article.appendChild(last);

    const actions = document.createElement('div');
    actions.className = 'a11y-profile__actions';

    clearButton = document.createElement('button');
    clearButton.type = 'button';
    clearButton.className = 'a11y-btn';
    clearButton.textContent = clearLabel;
    clearButton.addEventListener('click', () => {
      const profileToClear = getClearProfile();
      if(profileToClear){
        profileToClear.features.forEach(slug => toggleFeature(slug, false));
      }
      currentProfilePresetKey = '';
      persistLastProfileKey('');
      syncProfilePresetInstances();
      status.textContent = clearedStatus;
    });
    ...
    const instance = {
      article,
      sync: updateLastProfile
    };
    profilePresetInstances.add(instance);
    updateLastProfile();

    return article;
  }

  function getFeedbackActiveFeatures(){
    return Object.keys(featureState).filter(key => featureState[key]);
  }

  function createFeedbackCard(feature){
    const article = document.createElement('article');
    article.className = 'a11y-card a11y-card--feedback';
    article.setAttribute('data-role', 'feature-card');
    ...
    const labelEl = document.createElement('span');
    labelEl.className = 'label';
    labelEl.textContent = feature.label || 'Retour utilisateur';
    meta.appendChild(labelEl);
    ...
    if(intro){
      const introEl = document.createElement('p');
      introEl.className = 'a11y-feedback__intro';
      introEl.textContent = intro;
      article.appendChild(introEl);
    }

    const ratingName = `a11y-feedback-${Date.now()}-${Math.random().toString(36).slice(2)}`;

    const fieldset = document.createElement('fieldset');
    fieldset.className = 'a11y-feedback__fieldset';

    const legend = document.createElement('legend');
    legend.className = 'a11y-feedback__legend';
    legend.textContent = getFeatureSetting(feature, 'rating_label', 'Le réglage vous aide-t-il ?');
    fieldset.appendChild(legend);

    const choices = document.createElement('div');
    choices.className = 'a11y-feedback__choices';
    const ratingOptions = [
      ['yes', getFeatureSetting(feature, 'rating_yes', 'Oui')],
      ['partial', getFeatureSetting(feature, 'rating_partial', 'Partiellement')],
      ['no', getFeatureSetting(feature, 'rating_no', 'Non')]
    ];
    const ratingInputs = [];

    ratingOptions.forEach(([value, label]) => {
      const choiceLabel = document.createElement('label');
      choiceLabel.className = 'a11y-feedback-choice';
      const input = document.createElement('input');
      input.type = 'radio';
    ...
    const commentId = `a11y-feedback-comment-${Math.random().toString(36).slice(2)}`;
    const commentGroup = document.createElement('div');
    commentGroup.className = 'a11y-feedback__comment';

    const commentLabel = document.createElement('label');
    commentLabel.setAttribute('for', commentId);
    commentLabel.textContent = getFeatureSetting(feature, 'comment_label', 'Commentaire');
    ...
    comment.placeholder = getFeatureSetting(feature, 'comment_placeholder', '');
    commentGroup.appendChild(comment);
    article.appendChild(commentGroup);

    const consentId = `a11y-feedback-consent-${Math.random().toString(36).slice(2)}`;
    const consentLabel = document.createElement('label');
    consentLabel.className = 'a11y-feedback__consent';
    ...
    consentText.textContent = getFeatureSetting(feature, 'consent_label', 'J’accepte d’envoyer ce retour à l’équipe du site.');
    consentLabel.appendChild(consent);
    consentLabel.appendChild(consentText);
    article.appendChild(consentLabel);

    const honeypotGroup = document.createElement('div');
    honeypotGroup.className = 'a11y-feedback__honeypot';
    honeypotGroup.setAttribute('aria-hidden', 'true');
    const honeypotLabel = document.createElement('label');
    const honeypotId = `a11y-feedback-website-${Math.random().toString(36).slice(2)}`;
    honeypotLabel.setAttribute('for', honeypotId);
    honeypotLabel.textContent = 'Site web';
    ...
    honeypotGroup.appendChild(honeypot);
    article.appendChild(honeypotGroup);

    const note = document.createElement('p');
    note.className = 'a11y-feedback__note';
    note.textContent = getFeatureSetting(feature, 'privacy_note', 'Le retour est transmis à l’équipe du site.');
    article.appendChild(note);

    const status = document.createElement('div');
    status.className = 'a11y-feedback__status';
    status.setAttribute('role', 'status');
    status.setAttribute('aria-live', 'polite');
    const liveLabel = getFeatureSetting(feature, 'live_region_label', '');
    if(liveLabel){ status.setAttribute('aria-label', liveLabel); }
    ...
    const actions = document.createElement('div');
    actions.className = 'a11y-feedback__actions';

    const saveButton = document.createElement('button');
    saveButton.type = 'button';
    saveButton.className = 'a11y-btn primary';
    saveButton.textContent = getFeatureSetting(feature, 'save_label', 'Enregistrer');
    saveButton.addEventListener('click', async () => {
      const rating = getRating();
      if(!rating){
        status.textContent = getFeatureSetting(feature, 'empty_status', 'Choisissez une réponse avant d’enregistrer.');
        return;
      }
      if(!consent.checked){
        status.textContent = getFeatureSetting(feature, 'consent_status', 'Confirmez l’envoi du retour avant de continuer.');
        return;
      }
      const endpoint = getFeatureSetting(feature, 'submit_endpoint', '');
      const action = getFeatureSetting(feature, 'submit_action', 'a11y_widget_submit_feedback');
      const nonce = getFeatureSetting(feature, 'submit_nonce', '');
      if(!endpoint || !nonce || typeof fetch !== 'function'){
        status.textContent = getFeatureSetting(feature, 'submit_error_status', 'Impossible d’envoyer le retour pour le moment.');
        return;
      }
      ...
      payload.set('comment', comment.value.trim());
      payload.set('consent', '1');
      payload.set('page_url', window.location.href);
      payload.set('profile', getActiveProfileKey(loadLastProfileKey()) || '');
      payload.set('active_features', JSON.stringify(getFeedbackActiveFeatures()));
      payload.set('website', honeypot.value || '');
      saveButton.disabled = true;
      status.textContent = getFeatureSetting(feature, 'sending_status', 'Envoi du retour…');
      try {
        ...
        const message = json && json.data && typeof json.data.message === 'string'
          ? json.data.message
          : '';
        if(!response.ok || !json || json.success !== true){
          status.textContent = message || getFeatureSetting(feature, 'submit_error_status', 'Impossible d’envoyer le retour pour le moment.');
          return;
        }
        ...
        honeypot.value = '';
        status.textContent = message || getFeatureSetting(feature, 'saved_status', 'Retour transmis.');
      } catch(err){
        status.textContent = getFeatureSetting(feature, 'submit_error_status', 'Impossible d’envoyer le retour pour le moment.');
      } finally {
    ...
    clearButton.type = 'button';
    clearButton.className = 'a11y-btn';
    clearButton.textContent = getFeatureSetting(feature, 'clear_label', 'Effacer');
    clearButton.addEventListener('click', () => {
      ratingInputs.forEach(input => { input.checked = false; });
      comment.value = '';
      consent.checked = false;
      honeypot.value = '';
      status.textContent = getFeatureSetting(feature, 'cleared_status', 'Retour effacé.');
    });
    actions.appendChild(clearButton);
  ...
  function normalizeStatementUrl(value){
  ...
  const DEFAULT_PROJECT_CREATOR_NAMES = [
    'Amandine Bouteloup',
    'Marie Le Bout',
    'Noémie Masclef',
    'Miranda Oger',
  ...
  function createAccessibilityStatementCard(feature){
    const article = document.createElement('article');
    article.className = 'a11y-card a11y-card--statement';
    article.setAttribute('data-role', 'feature-card');
    ...
    const labelEl = document.createElement('span');
    labelEl.className = 'label';
    labelEl.textContent = feature.label || 'Déclaration d’accessibilité';
    meta.appendChild(labelEl);
    ...
    if(intro){
      const introEl = document.createElement('p');
      introEl.className = 'a11y-statement__intro';
      introEl.textContent = intro;
      article.appendChild(introEl);
    }

    const declarationUrl = normalizeStatementUrl(getFeatureSetting(feature, 'declaration_url', ''));
    const auditDate = formatStatementDate(getFeatureSetting(feature, 'audit_date', ''));
    const statusText = getFeatureSetting(feature, 'audit_status_text', '');
    const statusValue = getFeatureSetting(feature, 'audit_status', 'not_assessed');
    const hasPublicStatus = !!(statusText && statusValue !== 'not_assessed');
    const isConfigured = declarationUrl || hasPublicStatus || auditDate;

    if(!isConfigured){
      const empty = document.createElement('p');
      empty.className = 'a11y-statement__empty';
      empty.textContent = getFeatureSetting(feature, 'not_configured', 'La déclaration d’accessibilité n’est pas encore configurée pour ce site.');
      article.appendChild(empty);
      return article;
    }

    if(statusText && (hasPublicStatus || auditDate)){
      const statusParts = [`${getFeatureSetting(feature, 'status_label', 'Statut déclaré')} : ${statusText}`];
      if(auditDate){
        statusParts.push(`${getFeatureSetting(feature, 'date_label', 'Date de l’audit')} : ${auditDate}`);
      }
      const status = document.createElement('p');
      status.className = 'a11y-statement__status';
      status.setAttribute('data-status', statusValue || 'not_assessed');
      status.textContent = statusParts.join(' · ');
      article.appendChild(status);
    }

    const actions = document.createElement('div');
    actions.className = 'a11y-statement__actions';
    if(declarationUrl){
      const declarationLink = document.createElement('a');
      declarationLink.className = 'a11y-btn primary';
      declarationLink.href = declarationUrl;
      declarationLink.target = '_blank';
      declarationLink.rel = 'noopener';
      declarationLink.textContent = getFeatureSetting(feature, 'declaration_label', 'Ouvrir la déclaration');
      actions.appendChild(declarationLink);
    }
    if(actions.childNodes.length){
      article.appendChild(actions);
    }

    const sourceNote = getFeatureSetting(feature, 'source_note', '');
    if(sourceNote){
      const note = document.createElement('p');
      note.className = 'a11y-statement__note';
  ...
  function createProjectCreditsCard(feature){
    const article = document.createElement('article');
    article.className = 'a11y-card a11y-card--project-credits';
    article.setAttribute('data-role', 'feature-card');
    ...
    const labelEl = document.createElement('span');
    labelEl.className = 'label';
    labelEl.textContent = feature.label || getFeatureSetting(feature, 'credits_label', 'Créatrices de l’application');
    meta.appendChild(labelEl);
    ...
    if(intro){
      const introEl = document.createElement('p');
      introEl.className = 'a11y-project-credits__intro';
      introEl.textContent = intro;
      article.appendChild(introEl);
    }

    const creditsPeople = getFeatureSettingList(feature, 'credits_people');
    const names = creditsPeople.length
      ? creditsPeople
      : getFeatureSetting(feature, 'credits_names', DEFAULT_PROJECT_CREATOR_NAMES.join(', '))
        .split(',')
        .map(name => name.trim())
        .filter(Boolean);
    ...
    if(names.length){
      ...
      list.className = 'a11y-project-credits__list';
      list.setAttribute('aria-label', getFeatureSetting(feature, 'credits_label', 'Créatrices de l’application'));

      names.forEach(name => {
        const item = document.createElement('li');
        item.className = 'a11y-project-credits__item';
  ...
  const GLOSSARY_BUILTIN_TERMS = [
    {
      term: 'RGAA',
      aliases: ['référentiel général amélioration accessibilité'],
      definition: 'Référentiel français qui décrit comment vérifier l’accessibilité numérique d’un site ou service.'
    },
    {
      term: 'WCAG',
      aliases: ['web content accessibility guidelines'],
      definition: 'Règles internationales du W3C qui servent de base à de nombreux référentiels, dont le RGAA.'
    },
    {
      term: 'ARIA',
      aliases: ['wai aria', 'rôle aria'],
      definition: 'Ensemble d’attributs qui complètent la sémantique HTML quand un composant personnalisé en a besoin. ARIA ne corrige pas un HTML mal structuré.'
    },
    {
      term: 'Lecteur d’écran',
      aliases: ['screen reader'],
      definition: 'Technologie d’assistance qui restitue vocalement ou en braille la structure, les rôles, les états et le contenu d’une interface.'
    },
    {
      term: 'Focus',
      aliases: ['focus clavier'],
      definition: 'Emplacement actif du clavier dans la page. Il doit être visible et se déplacer dans un ordre cohérent.'
    },
    {
      term: 'Repère',
      aliases: ['landmark', 'région'],
      definition: 'Zone structurante de la page, par exemple navigation, contenu principal, recherche ou pied de page.'
    },
    {
      term: 'Texte alternatif',
      aliases: ['alt', 'alternative textuelle'],
      definition: 'Texte qui remplace une image quand elle ne peut pas être vue ou chargée, notamment avec un lecteur d’écran.'
    },
    {
      term: 'Contraste',
      aliases: ['ratio de contraste'],
      definition: 'Différence de luminosité entre le texte et son arrière-plan. Un contraste suffisant améliore la lisibilité.'
    },
    {
      term: 'TTS',
      aliases: ['text to speech', 'synthèse vocale', 'lecture audio'],
      definition: 'Lecture vocale de confort. Elle ne remplace pas un lecteur d’écran, car elle ne restitue pas toute la structure interactive.'
    },
    {
      term: 'Braille Unicode',
      aliases: ['braille visuel'],
      definition: 'Caractères braille affichés visuellement à l’écran. Cela ne signifie pas qu’une plage braille matérielle est pilotée.'
    },
    {
      term: 'localStorage',
      aliases: ['stockage local'],
      definition: 'Stockage local du navigateur. Il garde des préférences sur cet appareil et ce profil, sans envoi automatique au serveur.'
    },
    {
      term: 'Déclencheur visuel',
      aliases: ['flash', 'animation'],
      definition: 'Effet visuel potentiellement gênant, par exemple clignotement, mouvement rapide, autoplay ou variation lumineuse.'
    }
  ];

  function isWidgetOrAdminElement(element){
    return !!(element && element.closest && (element.closest('#a11y-widget-root') || element.closest('#wpadminbar')));
  }

  function isVisiblePageElement(element){
    if(!element || isWidgetOrAdminElement(element)){ return false; }
  ...
  function getCompactText(element, fallback=''){
  ...
  function getLabelledByText(element){
    const ids = element && element.getAttribute ? (element.getAttribute('aria-labelledby') || '') : '';
    if(!ids){ return ''; }
    return ids.split(/\s+/).map(id => {
      const target = document.getElementById(id);
      return target ? getCompactText(target, '') : '';
    }).filter(Boolean).join(' ');
  }

  function getLandmarkKind(element){
    if(!element){ return 'Repère'; }
  ...
  function getLandmarkName(element){
    ...
    const heading = element.querySelector ? element.querySelector('h1, h2, h3, h4, h5, h6') : null;
    if(heading){ return getCompactText(heading, ''); }
    return '';
  }

  function getPageLandmarks(){
    const selector = 'main, [role="main"], nav, [role="navigation"], header, [role="banner"], footer, [role="contentinfo"], aside, [role="complementary"], [role="search"], form[role="search"]';
    const seen = new Set();
    return Array.from(document.querySelectorAll(selector)).filter(element => {
      if(seen.has(element) || !isVisiblePageElement(element)){ return false; }
      seen.add(element);
      return true;
    }).slice(0, 16).map(element => ({
      element,
      kind: getLandmarkKind(element),
      name: getLandmarkName(element)
    }));
  }

  function getPageHeadings(){
    return Array.from(document.querySelectorAll('h1, h2, h3, h4, h5, h6')).filter(isVisiblePageElement).slice(0, 40).map(element => ({
      element,
      level: Number((element.tagName || 'H0').replace(/\D+/g, '')) || 0,
      text: getCompactText(element, 'Titre sans texte')
    }));
  }

  function focusPageTarget(element){
    if(!element || !document.contains(element)){ return; }
    if(!element.hasAttribute('tabindex')){
      element.setAttribute('tabindex', '-1');
      element.setAttribute('data-a11y-temp-focus-target', 'true');
    }
    try { element.scrollIntoView({ behavior: 'smooth', block: 'start' }); }
    catch(err){ element.scrollIntoView(); }
    try { element.focus({ preventScroll: true }); }
    catch(err){
      try { element.focus(); } catch(innerErr){ /* ignore */ }
    }
  }

  function goToPageTarget(element){
    const target = element;
    closePanel();
    window.setTimeout(() => focusPageTarget(target), 80);
  }

  function createStructureRow(item, options){
    const row = document.createElement('li');
    row.className = 'a11y-structure-row';

    const meta = document.createElement('div');
    meta.className = 'a11y-structure-row__meta';

    const title = document.createElement('span');
    title.className = 'a11y-structure-row__title';
    title.textContent = options.title;
    meta.appendChild(title);

    if(options.detail){
      const detail = document.createElement('span');
      detail.className = 'a11y-structure-row__detail';
    ...
    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'a11y-structure-row__action';
    button.textContent = options.goLabel;
    button.setAttribute('aria-label', `${options.goLabel} : ${options.title}`);
    button.addEventListener('click', () => goToPageTarget(item.element));
  ...
  function renderStructureList(list, items, emptyText, buildOptions){
    list.innerHTML = '';
    if(!items.length){
      const empty = document.createElement('li');
      empty.className = 'a11y-structure-empty';
      empty.textContent = emptyText;
      list.appendChild(empty);
      return;
    }
    items.forEach(item => list.appendChild(createStructureRow(item, buildOptions(item))));
  }

  function createPageStructureCard(feature){
    const article = document.createElement('article');
    article.className = 'a11y-card a11y-card--page-structure';
    article.setAttribute('data-role', 'feature-card');
    ...
    const labelEl = document.createElement('span');
    labelEl.className = 'label';
    labelEl.textContent = feature.label || 'Structure de page';
    meta.appendChild(labelEl);
    ...
    if(intro){
      const introEl = document.createElement('p');
      introEl.className = 'a11y-structure__intro';
      introEl.textContent = intro;
      article.appendChild(introEl);
    }

    const summary = document.createElement('p');
    summary.className = 'a11y-structure__summary';
    summary.setAttribute('role', 'status');
    summary.setAttribute('aria-live', 'polite');
    const liveLabel = getFeatureSetting(feature, 'live_region_label', '');
    if(liveLabel){ summary.setAttribute('aria-label', liveLabel); }
    article.appendChild(summary);

    const actions = document.createElement('div');
    actions.className = 'a11y-structure__actions';
    const refresh = document.createElement('button');
    refresh.type = 'button';
    refresh.className = 'a11y-btn';
    refresh.textContent = getFeatureSetting(feature, 'refresh_label', 'Actualiser');
    actions.appendChild(refresh);
    article.appendChild(actions);

    const sections = document.createElement('div');
    sections.className = 'a11y-structure-sections';

    const landmarksSection = document.createElement('section');
    landmarksSection.className = 'a11y-structure-section';
    const landmarksTitle = document.createElement('h4');
    landmarksTitle.className = 'a11y-structure-section__title';
    landmarksTitle.textContent = getFeatureSetting(feature, 'landmarks_label', 'Repères');
    const landmarksList = document.createElement('ul');
    landmarksList.className = 'a11y-structure-list';
    ...
    headingsSection.className = 'a11y-structure-section';
    const headingsTitle = document.createElement('h4');
    headingsTitle.className = 'a11y-structure-section__title';
    headingsTitle.textContent = getFeatureSetting(feature, 'headings_label', 'Titres');
    const headingsList = document.createElement('ul');
    headingsList.className = 'a11y-structure-list';
    headingsSection.appendChild(headingsTitle);
    headingsSection.appendChild(headingsList);

    sections.appendChild(landmarksSection);
    ...
    const update = () => {
      ...
      const headingWord = headings.length > 1 ? 'titres' : 'titre';
      const landmarkWord = landmarks.length > 1 ? 'repères' : 'repère';
      const summaryLabel = getFeatureSetting(feature, 'summary_label', 'Résumé de structure');
      summary.textContent = `${summaryLabel} : ${headings.length} ${headingWord}, ${landmarks.length} ${landmarkWord}.`;
      renderStructureList(
        landmarksList,
        landmarks,
        getFeatureSetting(feature, 'empty_landmarks', 'Aucun repère principal détecté dans la page.'),
        item => ({
          title: item.name ? `${item.kind} : ${item.name}` : item.kind,
          detail: '',
          goLabel: getFeatureSetting(feature, 'go_label', 'Aller')
        })
      );
      renderStructureList(
        headingsList,
        headings,
        getFeatureSetting(feature, 'empty_headings', 'Aucun titre détecté dans le contenu de la page.'),
        item => ({
          title: item.text,
          detail: `${getFeatureSetting(feature, 'heading_level_text', 'Niveau')} ${item.level}`,
          goLabel: getFeatureSetting(feature, 'go_label', 'Aller')
        })
      );
    };
  ...
  function getPageGlossaryTerms(){
    const terms = [];
    const seen = new Set();
    const addTerm = (term, definition, source='page') => {
      const cleanTerm = typeof term === 'string' ? term.replace(/\s+/g, ' ').trim() : '';
      const cleanDefinition = typeof definition === 'string' ? definition.replace(/\s+/g, ' ').trim() : '';
      if(!cleanTerm || !cleanDefinition){ return; }
      const key = normalizeString(cleanTerm);
      if(!key || seen.has(key)){ return; }
      seen.add(key);
      terms.push({ term: cleanTerm, definition: cleanDefinition, source });
    };

    Array.from(document.querySelectorAll('abbr[title]')).forEach(element => {
      if(isVisiblePageElement(element)){
        addTerm(getCompactText(element, ''), element.getAttribute('title') || '', 'page');
      }
    });

    Array.from(document.querySelectorAll('dfn[title]')).forEach(element => {
      if(isVisiblePageElement(element)){
        addTerm(getCompactText(element, ''), element.getAttribute('title') || '', 'page');
      }
    });

    Array.from(document.querySelectorAll('[data-glossary-term][data-definition], [data-term][data-definition]')).forEach(element => {
      if(isVisiblePageElement(element)){
        addTerm(element.getAttribute('data-glossary-term') || element.getAttribute('data-term') || getCompactText(element, ''), element.getAttribute('data-definition') || '', 'page');
      }
    });
  ...
  function glossaryTermMatches(entry, query){
    if(!query){ return true; }
    const aliases = Array.isArray(entry.aliases) ? entry.aliases.join(' ') : '';
    return matchesText(entry.term, query)
      || matchesText(entry.definition, query)
      || matchesText(aliases, query);
  }

  function renderGlossaryGroup(list, terms, emptyText){
    list.innerHTML = '';
    if(!terms.length){
      const empty = document.createElement('li');
      empty.className = 'a11y-glossary-empty';
      empty.textContent = emptyText;
      list.appendChild(empty);
      return;
    }
    terms.forEach(entry => {
      const item = document.createElement('li');
      item.className = 'a11y-glossary-term';
      const term = document.createElement('span');
      term.className = 'a11y-glossary-term__name';
      term.textContent = entry.term;
      const definition = document.createElement('span');
      definition.className = 'a11y-glossary-term__definition';
  ...
  function createGlossaryCard(feature){
    const article = document.createElement('article');
    article.className = 'a11y-card a11y-card--glossary';
    article.setAttribute('data-role', 'feature-card');
    ...
    const labelEl = document.createElement('span');
    labelEl.className = 'label';
    labelEl.textContent = feature.label || 'Dictionnaire / glossaire';
    meta.appendChild(labelEl);
    ...
    if(intro){
      const introEl = document.createElement('p');
      introEl.className = 'a11y-glossary__intro';
      introEl.textContent = intro;
      article.appendChild(introEl);
    }

    const searchId = `a11y-glossary-search-${Math.random().toString(36).slice(2)}`;
    const searchGroup = document.createElement('div');
    searchGroup.className = 'a11y-glossary-search';
    const searchLabel = document.createElement('label');
    searchLabel.setAttribute('for', searchId);
    searchLabel.textContent = getFeatureSetting(feature, 'search_label', 'Terme à chercher');
    const searchInputEl = document.createElement('input');
    searchInputEl.id = searchId;
    searchInputEl.type = 'search';
    searchInputEl.autocomplete = 'off';
    searchInputEl.placeholder = getFeatureSetting(feature, 'search_placeholder', '');
    searchGroup.appendChild(searchLabel);
    searchGroup.appendChild(searchInputEl);
    article.appendChild(searchGroup);

    const actions = document.createElement('div');
    actions.className = 'a11y-glossary__actions';
    const selectionButton = document.createElement('button');
    selectionButton.type = 'button';
    selectionButton.className = 'a11y-btn';
    selectionButton.textContent = getFeatureSetting(feature, 'selection_label', 'Utiliser la sélection');
    const clearButton = document.createElement('button');
    clearButton.type = 'button';
    clearButton.className = 'a11y-btn';
    clearButton.textContent = getFeatureSetting(feature, 'clear_label', 'Effacer');
    ...
    const status = document.createElement('div');
    status.className = 'a11y-glossary__status';
    status.setAttribute('role', 'status');
    status.setAttribute('aria-live', 'polite');
    const liveLabel = getFeatureSetting(feature, 'live_region_label', '');
    if(liveLabel){ status.setAttribute('aria-label', liveLabel); }
    ...
    groups.className = 'a11y-glossary-groups';

    const builtinSection = document.createElement('section');
    builtinSection.className = 'a11y-glossary-group';
    const builtinTitle = document.createElement('h4');
    builtinTitle.className = 'a11y-glossary-group__title';
    builtinTitle.textContent = getFeatureSetting(feature, 'builtin_label', 'Glossaire intégré');
    const builtinList = document.createElement('ul');
    builtinList.className = 'a11y-glossary-list';
    builtinSection.appendChild(builtinTitle);
    builtinSection.appendChild(builtinList);

    const pageSection = document.createElement('section');
    pageSection.className = 'a11y-glossary-group';
    const pageTitle = document.createElement('h4');
    pageTitle.className = 'a11y-glossary-group__title';
    pageTitle.textContent = getFeatureSetting(feature, 'page_label', 'Termes trouvés dans la page');
    const pageList = document.createElement('ul');
    pageList.className = 'a11y-glossary-list';
    pageSection.appendChild(pageTitle);
    pageSection.appendChild(pageList);

    groups.appendChild(builtinSection);
    groups.appendChild(pageSection);
    article.appendChild(groups);
    article.appendChild(status);

    const update = () => {
      const query = normalizeSearchQuery(searchInputEl.value || '');
      const pageTerms = getPageGlossaryTerms();
      const builtinMatches = GLOSSARY_BUILTIN_TERMS.filter(entry => glossaryTermMatches(entry, query));
      const pageMatches = pageTerms.filter(entry => glossaryTermMatches(entry, query));
      const emptyText = getFeatureSetting(feature, 'empty_label', 'Aucun terme correspondant pour le moment.');
      renderGlossaryGroup(builtinList, builtinMatches, emptyText);
      renderGlossaryGroup(pageList, pageMatches, emptyText);
      status.textContent = getFeatureSetting(feature, 'results_status', 'Résultats du glossaire mis à jour.');
    };

    searchInputEl.addEventListener('input', update);
    selectionButton.addEventListener('click', () => {
      const selected = window.getSelection ? window.getSelection().toString().replace(/\s+/g, ' ').trim() : '';
      if(!selected){
        status.textContent = getFeatureSetting(feature, 'no_selection_status', 'Sélectionnez un mot dans la page, puis réessayez.');
        return;
      }
      searchInputEl.value = selected.length > 80 ? selected.slice(0, 80) : selected;
      update();
      try { searchInputEl.focus({ preventScroll: true }); } catch(err){ searchInputEl.focus(); }
    });
    clearButton.addEventListener('click', () => {
      searchInputEl.value = '';
      update();
  ...
  function createCustomFeature(feature){
    const template = typeof feature.template === 'string' ? feature.template : '';
    let instance = null;
    if(template === 'profile-presets'){
      instance = createProfilePresetsCard(feature);
    } else if(template === 'user-feedback'){
      instance = createFeedbackCard(feature);
    } else if(template === 'accessibility-statement'){
      instance = createAccessibilityStatementCard(feature);
    } else if(template === 'project-credits'){
      instance = createProjectCreditsCard(feature);
    } else if(template === 'page-structure'){
      instance = createPageStructureCard(feature);
    } else if(template === 'dictionary-glossary'){
      instance = createGlossaryCard(feature);
    } else if(template === 'dyslexie-highlighter'){
      instance = createDyslexiaCard(feature);
    } else if(template === 'migraine-relief'){
      instance = createMigraineCard(feature);
    } else if(template === 'epilepsy-protection'){
      instance = createEpilepsyCard(feature);
    } else if(template === 'brightness-settings'){
      instance = createBrightnessCard(feature);
    } else if(template === 'button-settings'){
      instance = createButtonCard(feature);
    } else if(template === 'cursor-settings'){
      instance = createCursorCard(feature);
    } else if(template === 'reading-guide'){
      instance = createReadingGuideCard(feature);
    } else if(template === TTS_TEMPLATE_NAME){
      instance = createTtsCard(feature);
    } else if(template === BRAILLE_TEMPLATE_NAME){
      instance = createBrailleCard(feature);
    } else {
      instance = createFeaturePlaceholder(feature);
    }
    return decorateFeatureInstance(instance, feature);
  }
  ...
  const SEARCH_SYNONYM_GROUPS = [
    {
      terms: ['mal aux yeux', 'fatigue visuelle', 'fatigue yeux', 'trop lumineux', 'lumiere forte', 'lumiere', 'ecran agressif'],
      targets: ['migraine', 'luminosite', 'confort visuel', 'contraste', 'atténuation']
    },
    {
      terms: ['lecture difficile', 'difficile a lire', 'je lis mal', 'ligne perdue', 'suivre le texte', 'article long'],
      targets: ['guide de lecture', 'dyslexie', 'espacement', 'syllabification', 'mode texte seul']
    },
    {
      terms: ['texte trop petit', 'petit texte', 'grossir le texte', 'taille du texte', 'zoom', 'police'],
      targets: ['taille', 'texte', 'dyslexie', 'espacement', 'lecture']
    },
    {
      terms: ['je veux ecouter', 'ecouter', 'audio', 'voix', 'lecture vocale', 'lire a voix haute'],
      targets: ['lecture audio', 'synthese vocale', 'text-to-speech', 'tts']
    },
    {
      terms: ['souris invisible', 'curseur invisible', 'perdre le curseur', 'pointeur', 'clic difficile'],
      targets: ['curseur', 'navigation facilitee', 'boutons renforces', 'moteur']
    },
    {
      terms: ['bouton peu visible', 'liens peu visibles', 'champ invisible', 'formulaire difficile'],
      targets: ['boutons renforces', 'contraste', 'focus', 'moteur']
    },
    {
      terms: ['flash', 'animation', 'gif', 'video qui bouge', 'mouvement', 'parallax'],
      targets: ['mouvement', 'declencheurs visuels', 'epilepsie', 'reduire tous les mouvements']
    },
    {
      terms: ['braille', 'plage braille', 'visionneuse braille', 'nvda braille'],
      targets: ['braille visuel', 'unicode', 'visionneuse braille']
    },
    {
      terms: ['daltonisme', 'couleurs confuses', 'je distingue mal les couleurs', 'couleur'],
      targets: ['daltonisme', 'vision', 'contraste']
    },
    {
      terms: ['plan de page', 'titres', 'sommaire', 'structure', 'navigation page'],
      targets: ['structure de page', 'titres', 'repères', 'guide de lecture']
    }
  ];

  function getSearchNeedles(normalizedQuery){
    if(!normalizedQuery){ return []; }
    const needles = [normalizedQuery];
    SEARCH_SYNONYM_GROUPS.forEach(group => {
      const terms = Array.isArray(group.terms) ? group.terms : [];
      const matchesGroup = terms.some(term => {
        const normalizedTerm = normalizeSearchQuery(term);
        return normalizedTerm && (normalizedQuery.includes(normalizedTerm) || normalizedTerm.includes(normalizedQuery));
      });
      if(!matchesGroup){ return; }
      (Array.isArray(group.targets) ? group.targets : []).forEach(target => {
        const normalizedTarget = normalizeSearchQuery(target);
        if(normalizedTarget && !needles.includes(normalizedTarget)){
          needles.push(normalizedTarget);
        }
      });
    });
    return needles;
  ...
  function matchesAnyText(value, needles){
    if(!Array.isArray(needles) || !needles.length){ return false; }
    return needles.some(needle => matchesText(value, needle));
  }

  function featureMatchesSearch(feature, section, needles){
    if(!feature){ return false; }
    const catalogEntry = typeof feature.slug === 'string' ? featureCatalog.get(feature.slug) : null;
    const searchText = catalogEntry ? catalogEntry.searchText : buildFeatureSearchText(feature, section);
    return matchesAnyText(searchText, needles);
  }
  ...
  function renderSearchResults(normalizedQuery){
    ...
    if(!normalizedQuery){ return; }
    const searchNeedles = getSearchNeedles(normalizedQuery);
    let total = 0;
    sectionsData.forEach(section => {
      if(!section){ return; }
      const sectionTitle = typeof section.title === 'string' ? section.title : '';
      const sectionSlug = typeof section.slug === 'string' ? section.slug : '';
      const sectionMatches = matchesAnyText(`${sectionTitle} ${sectionSlug}`, searchNeedles);
      const features = Array.isArray(section.features) ? section.features : [];
      features.forEach(feature => {
        if(!feature){ return; }
        const hasChildren = Array.isArray(feature.children) && feature.children.length;
        const featureMatches = sectionMatches || featureMatchesSearch(feature, section, searchNeedles);
        let instance = null;
        if(hasChildren){
          const matchingChildren = feature.children.filter(child => {
            return featureMatchesSearch(child, section, searchNeedles);
          });
          if(featureMatches){
            instance = (typeof feature.slug === 'string' && feature.slug === 'vision-daltonisme')
              ? createColorblindCard(feature)
              : createFeatureGroup(feature);
          } else if(matchingChildren.length){
            const subset = Object.assign({}, feature, { children: matchingChildren });
            instance = (typeof feature.slug === 'string' && feature.slug === 'vision-daltonisme')
              ? createColorblindCard(subset)
              : createFeatureGroup(subset);
          }
  ...
  function pruneDetachedFeatureInputs(){
    ...
    featureToggleButtons.forEach((button, key) => {
  ...
  function disableSearchMode(options = {}){
    ...
    if(sectionAccordion){
      sectionAccordion.removeAttribute('hidden');
      sectionAccordion.removeAttribute('aria-hidden');
    }
    if(searchResults){
  ...
  function enterSearchMode(normalizedQuery){
    ...
    if(sectionAccordion){
      sectionAccordion.setAttribute('aria-hidden', 'true');
      sectionAccordion.setAttribute('hidden', '');
    }
    if(searchResults){
  ...
  function renderSection(sectionId){
    ...
    if(!features.length){
      ...
      updatePanelFocusability(panel);
      return;
    }
    const fragment = document.createDocumentFragment();
    let renderedCount = 0;
    features.forEach(feature => {
      if(isDedicatedGlossaryFeature(feature)){
        return;
      }
      if(isDedicatedStructureFeature(feature)){
        return;
      }
  ...
  function syncFooterUtilityToggleState(){
    if(!footerUtilityToggle || !footerUtilitySectionId){ return; }
    const parts = getPanelParts(footerUtilitySectionId);
    const isOpen = !!(parts && parts.panel && !parts.panel.hidden && activeSectionId === footerUtilitySectionId);
    footerUtilityToggle.setAttribute('aria-expanded', isOpen ? 'true' : 'false');
    footerUtilityToggle.classList.toggle('is-active', isOpen);
  }

  function focusFooterUtilityPanel(){
    const parts = getPanelParts(footerUtilitySectionId);
    if(!parts || !parts.panel){ return; }
    const firstFocusable = getFocusableElements(parts.panel)[0] || parts.panel;
    if(firstFocusable && typeof firstFocusable.focus === 'function'){
      try { firstFocusable.focus({ preventScroll: true }); } catch(err){ firstFocusable.focus(); }
    }
    if(typeof parts.panel.scrollIntoView === 'function'){
      parts.panel.scrollIntoView({ block: 'nearest' });
    }
  }

  function focusSectionToggle(toggle){
    if(toggle && toggle.focus){ toggle.focus(); }
  }

  function collapseSection(triggerToggle){
    const fallback = triggerToggle || sectionToggles[0] || null;
    activeSectionId = null;
    sectionToggles.forEach(item => {
    ...
    syncFooterUtilityToggleState();
    if(triggerToggle){ focusSectionToggle(triggerToggle); }
  }

  function setActiveSection(toggle, opts={}){
    if(!toggle){
    ...
    if(opts.focus){ focusSectionToggle(toggle); }
    if(activeParts){
  ...
  function handleSectionToggleClick(toggle){
    if(!toggle){ return; }
    const sectionId = toggle.dataset.sectionId || '';
    const isCurrent = sectionId && sectionId === activeSectionId && toggle.getAttribute('aria-expanded') === 'true';
    if(isCurrent){
      collapseSection(toggle);
      return;
    }
    setActiveSection(toggle);
  }
  ...
  function setupSectionNavigation(){
    ...
    const initiallyExpanded = sectionToggles.find(toggle => toggle.getAttribute('aria-expanded') === 'true');
    if(initiallyExpanded){
      setActiveSection(initiallyExpanded);
    } else {
      setActiveSection(sectionToggles[0]);
    }
  }

  // ---------- Shortcuts dialog ----------
  function showShortcutsDialogBackdrop(){
    if(root){ root.classList.add('is-shortcuts-dialog-open'); }
    if(!shortcutsBackdrop){ return; }
    shortcutsBackdrop.hidden = false;
    shortcutsBackdrop.classList.add('is-open');
  }

  function hideShortcutsDialogBackdrop(){
    if(root){ root.classList.remove('is-shortcuts-dialog-open'); }
    if(!shortcutsBackdrop){ return; }
    shortcutsBackdrop.classList.remove('is-open');
    shortcutsBackdrop.hidden = true;
  }

  function updateShortcutsTriggerState(isOpen){
    if(!tutorialToggle){ return; }
    tutorialToggle.setAttribute('aria-expanded', isOpen ? 'true' : 'false');
    tutorialToggle.classList.toggle('is-active', isOpen);
    const label = isOpen ? tutorialTriggerCloseLabel : tutorialTriggerOpenLabel;
    if(label){
      tutorialToggle.setAttribute('aria-label', label);
      tutorialToggle.setAttribute('title', label);
      tutorialToggle.dataset.tooltip = label;
    }
  }

  function updateShortcutsDialogPosition(){
    if(!tutorialSection || !panel || panel.hidden){ return; }
    const margin = 12;
    const gap = 12;
    const viewportWidth = window.innerWidth || document.documentElement.clientWidth || 0;
    const viewportHeight = window.innerHeight || document.documentElement.clientHeight || 0;
    const panelRect = panel.getBoundingClientRect();
    const dialogRect = tutorialSection.getBoundingClientRect();
    const dialogWidth = dialogRect.width || Math.min(352, Math.max(0, viewportWidth - margin * 2));
    const dialogHeight = dialogRect.height || 0;
    const anchorSide = panelSide === 'left' ? 'right' : 'left';
    const preferredLeft = anchorSide === 'right'
      ? panelRect.right + gap
      : panelRect.left - dialogWidth - gap;
    const maxLeft = Math.max(margin, viewportWidth - dialogWidth - margin);
    const fitsPreferredSide = anchorSide === 'right'
      ? preferredLeft + dialogWidth <= viewportWidth - margin
      : preferredLeft >= margin;
    let left = fitsPreferredSide
      ? preferredLeft
      : Math.min(Math.max(panelRect.left, margin), maxLeft);
    const maxTop = Math.max(margin, viewportHeight - dialogHeight - margin);
    let top = Math.min(Math.max(panelRect.top, margin), maxTop);

    if(!Number.isFinite(left)){ left = margin; }
    if(!Number.isFinite(top)){ top = margin; }

    tutorialSection.style.left = `${Math.round(left)}px`;
    tutorialSection.style.top = `${Math.round(top)}px`;
    tutorialSection.style.right = 'auto';
    tutorialSection.dataset.anchorSide = anchorSide;
  }

  function openShortcutsDialog(){
    if(!tutorialSection || !tutorialToggle || tutorialToggle.disabled || shortcutsDialogIsOpen){ return; }
    closeInfoDialog({ restoreFocus: false, immediate: true });
    if(shortcutsDialogHideTimer !== null){
      clearTimeout(shortcutsDialogHideTimer);
      shortcutsDialogHideTimer = null;
    }
    if(tutorialList && !tutorialList.childElementCount){
      populateTutorialList();
    }
    if(tutorialToggle.disabled || !shortcutDisplayList.length){ return; }
    tutorialSection.hidden = false;
    tutorialSection.setAttribute('aria-hidden', 'false');
    shortcutsDialogIsOpen = true;
    showShortcutsDialogBackdrop();
    updateShortcutsTriggerState(true);
    updateShortcutsDialogPosition();
    requestAnimationFrame(() => {
      if(tutorialSection){
        updateShortcutsDialogPosition();
        tutorialSection.classList.add('is-open');
      }
    });
    const focusTarget = tutorialCloseBtn
      || tutorialSection.querySelector('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])')
      || tutorialSection;
  ...
  function closeShortcutsDialog(options = {}){
    ...
    if(!tutorialSection || !shortcutsDialogIsOpen){ return false; }
    if(shortcutsDialogHideTimer !== null){
      clearTimeout(shortcutsDialogHideTimer);
      shortcutsDialogHideTimer = null;
    }
    shortcutsDialogIsOpen = false;
    tutorialSection.setAttribute('aria-hidden', 'true');
    tutorialSection.classList.remove('is-open');
    hideShortcutsDialogBackdrop();
    updateShortcutsTriggerState(false);
    if(immediate){
      tutorialSection.hidden = true;
    } else {
      shortcutsDialogHideTimer = window.setTimeout(() => {
        if(tutorialSection && !shortcutsDialogIsOpen){ tutorialSection.hidden = true; }
        shortcutsDialogHideTimer = null;
      }, 180);
    }
    if(restoreFocus && tutorialToggle && typeof tutorialToggle.focus === 'function'){
      try { tutorialToggle.focus({ preventScroll: true }); } catch(err){ tutorialToggle.focus(); }
    }
    return true;
  }

  function trapShortcutsDialogFocus(event){
    if(event.key !== 'Tab' || !tutorialSection || !shortcutsDialogIsOpen || tutorialSection.hidden){ return; }
    const focusables = getFocusableElements(tutorialSection);
    if(!focusables.length){
      event.preventDefault();
      focusElementSafely(tutorialSection);
      return;
    }
  ...
  function computeDefaultInfoDialogPosition(rect){
    const viewportWidth = window.innerWidth || document.documentElement.clientWidth || 0;
    const viewportHeight = window.innerHeight || document.documentElement.clientHeight || 0;
    const width = rect && rect.width ? rect.width : Math.min(1040, viewportWidth ? viewportWidth - 32 : 1040);
    const height = rect && rect.height ? rect.height : Math.min(860, viewportHeight ? viewportHeight - 32 : 860);
  ...
  function setInfoDisclosureState(button, explicitState, options = {}){
    if(!button){ return; }
    ...
    const isTopLevelInfoToggle = button.matches && button.matches('[data-role="info-menu-toggle"]');
    if(nextExpanded && isTopLevelInfoToggle && closeSiblings && infoDialog){
      infoDialog.querySelectorAll('[data-role="info-menu-toggle"]').forEach(otherToggle => {
        if(otherToggle !== button){
          setInfoDisclosureState(otherToggle, false);
        }
      });
    }
  ...
  function getInfoSearchTopItems(){
    return infoDialog ? Array.from(infoDialog.querySelectorAll('.a11y-info-menu__item')) : [];
  }

  function getInfoSearchSubmenus(item){
    return item ? Array.from(item.querySelectorAll('.a11y-info-submenu')) : [];
  }

  function getInfoTopToggle(item){
    return item ? item.querySelector('[data-role="info-menu-toggle"]') : null;
  }

  function getInfoSectionOwnText(item){
    if(!item){ return ''; }
    const parts = [];
    const toggle = getInfoTopToggle(item);
    const intro = item.querySelector('.a11y-info-panel__intro');
    const heading = item.querySelector('.a11y-info-heading');
    if(toggle){ parts.push(toggle.textContent || ''); }
    if(intro){ parts.push(intro.textContent || ''); }
    if(heading){ parts.push(heading.textContent || ''); }
    return parts.join(' ');
  }

  function setInfoSearchStatus(message){
    if(infoSearchStatus){
      infoSearchStatus.textContent = message || '';
    }
  }

  function formatInfoSearchResultCount(count){
    const template = count === 1 ? infoSearchResultSingular : infoSearchResultPlural;
    return (template || '%d rubriques trouvées dans l’aide.').replace('%d', String(count));
  }

  function resetInfoSearch(options = {}){
    const { keepInput = false } = options;
    if(!infoDialog){ return; }
    infoDialog.classList.remove('is-info-searching');
    if(!keepInput && infoSearchInput){ infoSearchInput.value = ''; }
    if(infoSearchEmpty){ infoSearchEmpty.hidden = true; }
    setInfoSearchStatus('');

    getInfoSearchTopItems().forEach((item, itemIndex) => {
      item.hidden = false;
      item.classList.remove('is-info-search-match');
      setInfoDisclosureState(getInfoTopToggle(item), itemIndex === 0, { closeSiblings: false });

      getInfoSearchSubmenus(item).forEach((submenu, submenuIndex) => {
        submenu.hidden = false;
        submenu.classList.remove('is-info-search-match');
        const nestedToggle = submenu.querySelector('[data-role="info-submenu-toggle"]');
        setInfoDisclosureState(nestedToggle, itemIndex === 0 && submenuIndex === 0);
      });
    });
  }

  function updateInfoSearchResults(){
    if(!infoDialog || !infoSearchInput){ return; }
    const normalizedQuery = normalizeSearchQuery(infoSearchInput.value || '');
    if(!normalizedQuery){
      resetInfoSearch({ keepInput: true });
      return;
    }

    const topItems = getInfoSearchTopItems();
    const visibleTopItems = [];

    topItems.forEach(item => {
      const sectionOwnMatches = matchesText(getInfoSectionOwnText(item), normalizedQuery);
      let visibleSubmenuCount = 0;

      getInfoSearchSubmenus(item).forEach((submenu, submenuIndex) => {
        const submenuMatches = matchesText(submenu.textContent || '', normalizedQuery);
        const submenuVisible = sectionOwnMatches || submenuMatches;
        submenu.hidden = !submenuVisible;
        submenu.classList.toggle('is-info-search-match', submenuVisible);

        const nestedToggle = submenu.querySelector('[data-role="info-submenu-toggle"]');
        if(submenuVisible){
          visibleSubmenuCount++;
          setInfoDisclosureState(nestedToggle, sectionOwnMatches ? submenuIndex === 0 : true);
        } else {
          setInfoDisclosureState(nestedToggle, false);
        }
      });

      const itemVisible = sectionOwnMatches || visibleSubmenuCount > 0;
      item.hidden = !itemVisible;
      item.classList.toggle('is-info-search-match', itemVisible);
      if(itemVisible){
        visibleTopItems.push(item);
      }
    });

    const openItem = visibleTopItems.find(item => {
      const toggle = getInfoTopToggle(item);
      return toggle && toggle.getAttribute('aria-expanded') === 'true';
    }) || visibleTopItems[0] || null;

    topItems.forEach(item => {
      setInfoDisclosureState(getInfoTopToggle(item), item === openItem, { closeSiblings: false });
    });

    infoDialog.classList.add('is-info-searching');
    if(infoSearchEmpty){
      infoSearchEmpty.hidden = visibleTopItems.length > 0;
    }
    setInfoSearchStatus(visibleTopItems.length ? formatInfoSearchResultCount(visibleTopItems.length) : infoSearchEmptyText);
  }
  ...
  const infoDialogIsolationState = new Map();

  function showInfoDialogBackdrop(){
    if(root){ root.classList.add('is-info-dialog-open'); }
    if(!infoBackdrop){ return; }
    infoBackdrop.hidden = false;
    infoBackdrop.classList.add('is-open');
  }

  function hideInfoDialogBackdrop(){
    if(root){ root.classList.remove('is-info-dialog-open'); }
    if(!infoBackdrop){ return; }
    infoBackdrop.classList.remove('is-open');
    infoBackdrop.hidden = true;
  }

  function getInfoDialogIsolationTargets(){
    if(!panel || !infoDialog){ return []; }
    return Array.from(panel.children).filter(element => (
      element !== infoDialog
      && element !== infoBackdrop
      && element.nodeType === 1
    ));
  }

  function applyInfoDialogIsolation(){
    if(infoDialogIsolationState.size){ return; }
    getInfoDialogIsolationTargets().forEach(element => {
      infoDialogIsolationState.set(element, {
  ...
  function restoreInfoDialogIsolation(){
    infoDialogIsolationState.forEach((state, element) => {
      if(!state.hadInert){ element.removeAttribute('inert'); }
      else { element.setAttribute('inert', ''); }
      if(state.ariaHidden === null){ element.removeAttribute('aria-hidden'); }
      else { element.setAttribute('aria-hidden', state.ariaHidden); }
    });
    infoDialogIsolationState.clear();
    hideInfoDialogBackdrop();
  }
  ...
  function openInfoDialog(){
    ...
    if(infoTrigger){
      ...
      if(infoTriggerCloseLabel){
        infoTrigger.setAttribute('aria-label', infoTriggerCloseLabel);
        infoTrigger.setAttribute('title', infoTriggerCloseLabel);
        infoTrigger.dataset.tooltip = infoTriggerCloseLabel;
      }
      infoTrigger.classList.add('is-active');
    }
    const focusTarget = infoSearchInput
      || infoDialog.querySelector('[data-role="info-menu-toggle"]')
      || infoCloseBtn
      || infoDialog.querySelector('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])')
      || infoDialog;
    if(focusTarget && typeof focusTarget.focus === 'function'){
      try { focusTarget.focus({ preventScroll: true }); } catch(err){ focusTarget.focus(); }
    }
    applyInfoDialogIsolation();
  }
  ...
  function closeInfoDialog(options = {}){
    ...
    if(infoTrigger){
      ...
      if(infoTriggerOpenLabel){
        infoTrigger.setAttribute('aria-label', infoTriggerOpenLabel);
        infoTrigger.setAttribute('title', infoTriggerOpenLabel);
        infoTrigger.dataset.tooltip = infoTriggerOpenLabel;
      }
  ...
  function trapInfoDialogFocus(event){
    if(event.key !== 'Tab' || !infoDialog || !infoDialogIsOpen || infoDialog.hidden){ return; }
    const focusables = getFocusableElements(infoDialog);
    if(!focusables.length){
      event.preventDefault();
      focusElementSafely(infoDialog);
      return;
    }
    const first = focusables[0];
    const last = focusables[focusables.length - 1];
  ...
  // ---------- Focus trap ----------
  const FOCUSABLE_SELECTOR = 'a[href], area[href], button, input, select, textarea, summary, iframe, object, embed, audio[controls], video[controls], [contenteditable="true"], [tabindex]:not([tabindex="-1"])';
  let lastFocused = null;
  let previousBodyOverflow = null;
  let modalFocusGuardActive = false;
  const modalIsolationState = new Map();

  function isElementHiddenForFocus(el){
    if(!el || el.disabled || el.hasAttribute('disabled')){ return true; }
  ...
  function getFocusableElements(container){
    if(!container){ return []; }
    return Array.from(container.querySelectorAll(FOCUSABLE_SELECTOR)).filter(el => !isElementHiddenForFocus(el));
  }

  function updatePanelFocusability(sectionPanel){
    if(!sectionPanel){ return; }
    sectionPanel.removeAttribute('tabindex');
  }

  function updateCurrentPanelFocusability(){
    const parts = getPanelParts(activeSectionId);
    if(parts && parts.panel && !parts.panel.hidden){
      updatePanelFocusability(parts.panel);
    }
  }
  ...
  function focusElementSafely(el){
    ...
    try { el.focus({ preventScroll: true }); }
      ...
      try { el.focus(); }
      catch(innerErr){ return false; }
    }
    return true;
  }

  function focusInitialPanelElement(){
    if(!panel){ return; }
  ...
  function getModalIsolationTargets(){
    if(!root || !document.body){ return []; }
    const targets = [];
    const seen = new Set();
    const addTarget = element => {
      if(!element || seen.has(element)){ return; }
      if(element === root || element === overlay){ return; }
    ...
    while(parent && parent !== document.body && parent !== document.documentElement){
      Array.from(parent.children).forEach(sibling => {
        if(sibling !== child){ addTarget(sibling); }
      });
      child = parent;
  ...
  function applyModalIsolation(){
    if(!isModalBackground || modalIsolationState.size){ return; }
    getModalIsolationTargets().forEach(element => {
      modalIsolationState.set(element, {
        hadInert: element.hasAttribute('inert'),
        ariaHidden: element.getAttribute('aria-hidden'),
      });
      element.setAttribute('inert', '');
  ...
  function restoreModalIsolation(){
    modalIsolationState.forEach((state, element) => {
      if(!state.hadInert){ element.removeAttribute('inert'); }
      else { element.setAttribute('inert', ''); }
      if(state.ariaHidden === null){ element.removeAttribute('aria-hidden'); }
      else { element.setAttribute('aria-hidden', state.ariaHidden); }
    });
    modalIsolationState.clear();
  }

  function handleModalFocusEscape(event){
    if(!isModalBackground || !panel || panel.hidden){ return; }
    const target = event.target;
    if(target && panel.contains(target)){ return; }
    focusInitialPanelElement();
  }

  function enableModalFocusGuard(){
    if(!isModalBackground || modalFocusGuardActive){ return; }
    document.addEventListener('focusin', handleModalFocusEscape, true);
    modalFocusGuardActive = true;
  }

  function disableModalFocusGuard(){
    if(!modalFocusGuardActive){ return; }
    document.removeEventListener('focusin', handleModalFocusEscape, true);
    modalFocusGuardActive = false;
  }

  function lockBodyScroll(){
    if(!isModalBackground || !document.body){ return; }
  ...
  function restoreBodyScroll(){
    ...
    if(isModalBackground){ document.body.style.removeProperty('overflow'); }
  }
  ...
  function openPanel(){
    ...
    syncAllRenderedCardStates();
    updateActiveSummary();
    updateCurrentPanelFocusability();
    focusInitialPanelElement();
  ...
  function closePanel(){
    ...
    if(lastFocused && document.contains(lastFocused)){ focusElementSafely(lastFocused); }
    lastFocused = null;
  }
  function trap(e){
    if(e.key === 'Escape'){
      const didCloseShortcuts = closeShortcutsDialog({ restoreFocus: true });
      if(didCloseShortcuts){ e.preventDefault(); return; }
    ...
    if(e.key !== 'Tab') return;
    if(infoDialogIsOpen && infoDialog && infoDialog.contains(e.target)){ return; }
    if(shortcutsDialogIsOpen && tutorialSection && tutorialSection.contains(e.target)){ return; }
    if(!panel){ return; }
    const focusables = getFocusableElements(panel);
    if(!focusables.length){ e.preventDefault(); focusElementSafely(panel); return; }
    const first = focusables[0];
    const last = focusables[focusables.length-1];
    if(e.shiftKey && document.activeElement === first){ e.preventDefault(); focusElementSafely(last); }
    else if(!e.shiftKey && document.activeElement === last){ e.preventDefault(); focusElementSafely(first); }
  }
  ...
  function toggleFeature(key, on, opts={}){
    const previous = !!featureState[key];
    ...
    if(opts.syncInput !== false){
      syncRenderedFeatureControls(key, on);
    }

    if(!opts.silent && opts.recordUndo !== false && !isApplyingUndo && previous !== !!on){
      lastUndoAction = { key, previous, next: !!on };
      syncUndoButton();
    }
  ...
  function resetWidgetPreferences(){
    const keys = new Set([...featureInputs.keys(), ...featureToggleButtons.keys(), ...Object.keys(featureState)]);
    keys.forEach(key => toggleFeature(key, false, { recordUndo: false }));
    lastUndoAction = null;
    featureInputs.forEach(input => {
      input.checked = false;
      syncFeatureControlCardState(input);
    });
    featureToggleButtons.forEach(button => updateFeatureToggleButton(button, false));
    featureState = {};
    ...
    resetEpilepsySettings({ persist: false });
    resetColorblindSettings({ persist: false });
    resetBrailleSettings({ persist: false });
    resetTtsSettings({ persist: false });
    resetMigraineSettings({ persist: false });
    resetBrightnessSettings({ persist: false });
    resetDyslexiaSettings({ persist: false });
    ...
    [
      ...
      PANEL_SIDE_KEY,
      PROFILE_PRESET_KEY,
      BUTTONS_SETTINGS_KEY,
      CURSOR_SETTINGS_KEY,
      BRIGHTNESS_SETTINGS_KEY,
      READING_GUIDE_SETTINGS_KEY,
      READING_GUIDE_SUMMARY_POS_KEY,
      COLORBLIND_SETTINGS_STORAGE_KEY,
      INFO_DIALOG_POS_KEY,
      EPILEPSY_SETTINGS_KEY,
      BRAILLE_SETTINGS_KEY,
      TTS_STORAGE_KEY,
      MIGRAINE_SETTINGS_KEY,
      DYSLEXIA_SETTINGS_KEY,
    ].forEach(storageKey => {
  ...
  if(tutorialToggle && tutorialSection){
    tutorialToggle.addEventListener('click', () => {
      ...
      if(shortcutsDialogIsOpen){
        closeShortcutsDialog({ restoreFocus: true });
      } else {
        openShortcutsDialog();
      }
    });
  ...
  if(structureToggle && structureView){
  ...
  if(structureCloseBtn){
    structureCloseBtn.addEventListener('click', () => {
      closeStructureView({ restoreFocus: true });
    });
  }
  ...
  if(tutorialSection){
    tutorialSection.addEventListener('keydown', event => {
      if(event.key === 'Escape'){
        const closed = closeShortcutsDialog({ restoreFocus: true });
        if(closed){
          event.preventDefault();
          event.stopPropagation();
        }
      ...
      trapShortcutsDialogFocus(event);
    });
  }

  if(infoSearchForm){
    infoSearchForm.addEventListener('submit', event => {
      event.preventDefault();
    });
  }
  if(infoSearchInput){
    infoSearchInput.addEventListener('input', updateInfoSearchResults);
    infoSearchInput.addEventListener('search', updateInfoSearchResults);
  }
  ...
  if(infoBackdrop){
    infoBackdrop.addEventListener('click', () => {
      closeInfoDialog({ restoreFocus: true });
    });
  }
  if(infoDialog){
  ...
  if(btn){
    ...
    btn.addEventListener('keydown', handleLauncherKeydown);
  ...
  if(footerUtilityToggle && footerUtilitySectionId){
    footerUtilityToggle.addEventListener('click', () => {
      disableSearchMode();
      const toggle = sectionToggles.find(item => (item.dataset.sectionId || '') === footerUtilitySectionId);
      if(!toggle){ return; }
      setActiveSection(toggle);
      focusFooterUtilityPanel();
    });
  }
  if(resetBtn){
    resetBtn.addEventListener('click', resetWidgetPreferences);
  }
  if(activeResetBtn){
    activeResetBtn.addEventListener('click', () => {
      if(activeResetBtn.disabled){ return; }
      resetWidgetPreferences();
    });
  }
  if(activeUndoBtn){
    activeUndoBtn.addEventListener('click', () => {
      if(activeUndoBtn.disabled){ return; }
      undoLastFeatureChange();
    });
  }
