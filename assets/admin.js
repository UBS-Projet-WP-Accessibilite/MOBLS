(function () {
    ...
    function updateProfileConfigCard(card) {
        if (!card) {
            return;
        }

        var enabledInput = card.querySelector('[data-profile-enabled]');
        var enabled = !enabledInput || enabledInput.checked;
        var checkedFeatures = [];

        card.classList.toggle('is-disabled', !enabled);

        toArray(card.querySelectorAll('[data-profile-feature-choice]')).forEach(function (input) {
            if (!input.checked) {
                return;
            }

            var label = input.getAttribute('data-profile-feature-label') || input.value || '';
            if (label) {
                checkedFeatures.push(label);
            }
        });

        var summary = card.querySelector('[data-profile-summary]');
        if (summary) {
            summary.textContent = checkedFeatures.join(', ');
        }

        var empty = card.querySelector('[data-profile-summary-empty]');
        if (empty) {
    ...
    function initProfileConfigCards(cards) {
        toArray(cards).forEach(function (card) {
            card.addEventListener('change', function (event) {
                ...
                if (
                    ...
                    && target.matches
                    && (
                        target.matches('[data-profile-enabled]')
                        || target.matches('[data-profile-feature-choice]')
                    )
                ) {
                    updateProfileConfigCard(card);
                }
            });
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        var launcherGroups = toArray(document.querySelectorAll('[data-launcher-choice-group]'));
        var visualThemeGroups = toArray(document.querySelectorAll('[data-visual-theme-group]'));
        ...
        var colorControls = toArray(document.querySelectorAll('.a11y-widget-admin-appearance__color-control input[type="color"]'));
        var profileConfigCards = toArray(document.querySelectorAll('[data-profile-config-card]'));

        initProfileConfigCards(profileConfigCards);

        colorControls.forEach(function (input) {
        ...
        visualThemeGroups.forEach(function (group) {
            function syncVisualThemeState() {
                toArray(group.querySelectorAll('.a11y-widget-admin-appearance__preset')).forEach(function (label) {
                    label.classList.remove('a11y-widget-admin-appearance__preset--checked');
                });

                toArray(group.querySelectorAll('[data-visual-theme-choice]')).forEach(function (input) {
                    var label = input.closest('.a11y-widget-admin-appearance__preset');

                    if (input.checked && label) {
                        label.classList.add('a11y-widget-admin-appearance__preset--checked');
                    }
                });
            }

            group.addEventListener('change', syncVisualThemeState);
            syncVisualThemeState();
        });
        ...
        launcherGroups.forEach(function (group) {
            group.addEventListener('change', function (event) {
                ...
                if (!target || !target.matches('[data-launcher-choice]')) {
                    return;
                }
                ...
                toArray(group.querySelectorAll('[data-launcher-choice]')).forEach(function (input) {
